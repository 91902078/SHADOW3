...
#define MP_A_NEW   
#if DEFINED(MP_POLARIZATION)   || DEFINED(MP_INTERCEPT) || defined(MP_A_NEW)
   use mpmodule, gamma_mp =>gamma, MP_PI =>MPPI
#endif
   implicit none
...
!!ccc    real(kind=skr),dimension(10) :: ccc
   ...
   real(kind=skr),dimension(0:MAX_NDEG,0:MAX_NDEG,0:MAX_NDEG) :: pcoeff
...
!----------------------------------------------------------------------------------------------------
      ...
      integer(kind=ski) :: i,nener,nrefl,i_latt, ierr,m_k
#if DEFINED(MP_POLARIZATION) || defined(MP_A_NEW)     
      type (mp_complex):: MP_RCS1,MP_RCS2,MP_RCP1,MP_RCP2,MP_ETA_S,MP_ETA_P 
#endif
      !
      ! new crystals
      !
...
604            CONTINUE
      ...
      ELSE ! normal calculation
         if (Q_PHOT .ne. 0.0 .and. kwhat .eq. 1) then
               m_k = m_order
         end if      
         ...
         if (i_file_refl_version .eq. 2) then
            ...
            SIN_GRA	  = m_k*R_LAM0/D_SPACING/2.0D0   !SSLS:Xiaojiang
            GRAZE	  = ASIN(SIN_GRA)
...
101         NENER = I - 1
            ...
            SIN_GRA	  = m_k*R_LAM0/D_SPACING/2.0D0
            GRAZE	  = ASIN(SIN_GRA)
            ASS_FAC	  = SIN_Q_ANG/SIN_Q_REF
            ! C
            ! C Interpolation
            ...
            IF (I_LATT .GT. -1 ) THEN                    ! True for normal crystal
            ...
            if (f_refrac.eq.0) then
               THETA_B = m_k*R_LAM0/(1-(DELTA_REF/SIN_GRA**2))/2.0D0/D_SPACING   !SSLS: Xiaojiang
               THETA_B = ASIN(THETA_B)
               ! C
               ! C Now THETA_B is the Bragg angle corrected for refraction for asym case
               ! C following Handbook of SR
         ...
         ELSE
            ...
            ! C SSLS: Xiaojiang, m_k
            ALPHA_ZAC =-((m_k*R_LAM0/D_SPACING)**2-2*m_k*R_LAM0* &
               SIN_BRG/D_SPACING)
            ! D
            ...
            IF (F_REFRAC.GT.0) THEN
               ...
               ! C
               gamma_0 = sin_q_ang     !SSLS:YXJ  removed -1d0
               ...
               !cry_alpha = -((r_lam0/d_spacing)**2+2D0*r_lam0* &       !SSLS:YXJ
               !   sin_brg/d_spacing)
               cry_alpha = -ALPHA_ZAC   
...
!                  br_delta2
               br_c1 = -1.d0*ci*thickness*user_units_to_cm*twopi/(abs(gamma_0))/r_lam0* &
                  br_delta1
               br_c2 = -1.d0*ci*thickness*user_units_to_cm*twopi/(abs(gamma_0))/r_lam0* &
                  br_delta2
               ! C
               ...
               !rcs = rcs/sqrt(abs(cry_b))         !SSLS:YXJ, moved to close to rcp
               ! C
               ! C p-polarization
               ! C
               c_ppol = abs(cos(2.0d0*graze))   !SSLS:YXJ, removed 'torad*'
               ...
               br_delta1 = 0.5d0*(psi_0-cry_z+ctemp)
               br_delta2 = 0.5d0*(psi_0-cry_z-ctemp)
               !br_c1 = -1.0d0*ci*thickness*user_units_to_cm*twopi/(-1.0d0*abs(gamma_0))/r_lam0* & !SSLS:YXJ
               !   br_delta1
               !br_c2 = -1.0d0*ci*thickness*user_units_to_cm*twopi/(-1.0d0*abs(gamma_0))/r_lam0* & !SSLS:YXJ
               !   br_delta2
               br_c1 = -1.0d0*ci*thickness*user_units_to_cm*twopi/(abs(gamma_0))/r_lam0* &
                  br_delta1
               br_c2 = -1.0d0*ci*thickness*user_units_to_cm*twopi/(abs(gamma_0))/r_lam0* &
                  br_delta2
               ! C
               ! C a very big exponential produces numerical overflow. If so the value
               ...
               ! Cc	if (f_refrac.eq.1) then
               !rcp = br_x1*br_x2*(br_c1-br_c2)/(br_x2-br_x1)              ! laue, SSLS:YXJ
               if (F_REFRAC.EQ.1) then
               ...
               IF (F_REFRAC.LE.0) then    !SSLS:YXJ
...
#ifdef MP_A_NEW  !SSLS: YXJ, reduce floating error
               MP_ETA_S = mpcmplx(MPREAL(DREAL(ETA_S)),MPREAL(DIMAG(ETA_S)))
               MP_ETA_P = mpcmplx(MPREAL(DREAL(ETA_P)),MPREAL(DIMAG(ETA_P)))
               MP_RCS1 = MP_ETA_S+SQRT(MP_ETA_S**2-1d0)
               MP_RCP1=  MP_ETA_P+SQRT(MP_ETA_P**2-1d0)
               MP_RCS2=  MP_ETA_S-SQRT(MP_ETA_S**2-1d0)
               MP_RCP2=  MP_ETA_P-SQRT(MP_ETA_P**2-1d0)
               RCS1 = MP_RCS1
               RCS2 = MP_RCS2
               RCP1 = MP_RCP1
               RCP2 = MP_RCP2
...
1220  FORMAT (1X,'Vector orthogonal to grooves at (0,0,0) : ',/, &
   ...
   Subroutine IMAGE1  (RAY,AP,PHASE,I_WHAT)
      ...
      real(kind=skr),dimension(: ,:),allocatable ::  RAY_STORE
      real(kind=skr),dimension(:,:),allocatable ::  PLATE
      ...
      integer(kind=ski)               :: j, icheck, iflag, jj, ierr, kounts,i
      ...
      allocate(RAY_STORE(6,NPOINT),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"IMAGE1: Allocate RAY_STORE failure. Stop!";STOP __LINE__
      end if
      allocate(PLATE(12,NPOINT),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"IMAGE1: Allocate PLATE failure. Stop!";STOP __LINE__
      end if
...
9990  FORMAT (I4.4)
      ...
      deallocate(RAY_STORE);deallocate(PLATE)
   ...
   Subroutine INTERCEPT (XIN, VIN, TPAR, IFLAG)
      ...
      ELSE IF (FMIRR.EQ.9) THEN ! Gen poly
         IF (F_KOMA.NE.1) THEN
            ...
            ELSE
               IF (F_CONVEX.NE.0) IFLAG = -1
               CALL MP_POLY ( XIN, VIN, TPAR, IFLAG)
            END if
...
! see http://ftp.esrf.fr/pub/scisoft/shadow/user_contributions/hyperbola_fixes_2008-10-22.txt
         ...
         ELSE
            ...
            ! avoid loss of significance B ~ DETER, that happens if AA<<1
            !TPAR1       = -(BB + SQRT(DETER))*DENOM
            !TPAR2       = -(BB - SQRT(DETER))*DENOM
            if(ABS(4*CC*AA/BB/BB).ge.1.0D-7) then  ! normal case, 1d-6 to 1d-7
               TPAR1 = -(BB + SQRT(DETER))*DENOM
               TPAR2 = -(BB - SQRT(DETER))*DENOM
            else  ! patological case, e.g., in parabolic lenses close to the pole
               !TPAR1 = - CC/BB - AA*CC**2/BB**3
               !TPAR2 = -(BB/AA + TPAR1)
               TPAR1 = (-BB + abs(BB))*DENOM - CC/BB - AA*CC**2/BB**3
               TPAR2 = -(BB/AA + TPAR1)
            endif
         END IF
...
! C
   ...
   Subroutine SPOLY (XIN, VIN, ANSWER, I_RES)
      ...
      !COMPLEX*16	H_OUTPUT(4)
      COMPLEX*16	H_OUTPUT(MAX_NDEG)      !yxj, polynomial degree changed

      !srio danger  (to test)
      !real(kind=skr),dimension(8)  :: h_output88
      real(kind=skr),dimension(MAX_NDEG*2)  :: h_output88     !yxj, 4*2 to MAX_NDEG*2
      equivalence (h_output88(1), H_OUTPUT(1))

      INTEGER(KIND=ski) N_TEST
      !DIMENSION	TEST1(4),TEST2(4)
      DIMENSION	TEST1(MAX_NDEG),TEST2(MAX_NDEG)
      DIMENSION	XIN(3), VIN(3)
      !DIMENSION	COEFF(5),TCOEFF(5)
      DIMENSION	COEFF(MAX_NDEG+1),TCOEFF(MAX_NDEG+1)
      ANSWER	=   0.0D0
      X0	= XIN(1)
      Y0	= XIN(2)
      Z0	= XIN(3)
                  ...
                  DO I1=0,I
                     DO J1=0,J
                        DO K1=0,K
                           ...
                           I_TMP2 = 0
                           IF (I_TMP1 .GT. 0) THEN
                              VTEMP = VTEMP * X0 **I_TMP1
                              DO WHILE (I_TMP1 .GT. 0 )
                                 NI1 = NI1 * (I - I_TMP2)
                                 NI2 = NI2 * (1 + I_TMP2)
                                 I_TMP1 = I_TMP1 - 1
                                 I_TMP2 = I_TMP2 + 1
                           ...
                           IJK1 = I1 + J1 + K1 + 1
                           IF (I1 .GT. 0) VTEMP = VTEMP * X1**I1
                           IF (J1 .GT. 0) VTEMP = VTEMP * Y1**J1
                           IF (K1 .GT. 0) VTEMP = VTEMP * Z1**K1
                           TCOEFF(IJK1) = TCOEFF(IJK1) + PCOEFF(I,J,K)*VTEMP*  &
...
!todo srio: is this the same as spoly?
   ...
   SUBROUTINE POLY (XIN, VIN, ANSWER, I_RES)
      ...
      COMPLEX*16	H_OUTPUT(MAX_NDEG)

      !srio danger  (to test)
      real(kind=skr),dimension(MAX_NDEG*2)  :: h_output88
      equivalence (h_output88(1), H_OUTPUT(1))
               ...
               DO I1=0,I
                  DO J1=0,J
                     DO 7 K1=0,K
                        ...
                        I_TMP1 = I - I1
                        I_TMP2 = 0
                        IF (I_TMP1 .GT. 0 .AND. X0 .NE. 0.0D0) THEN
                           VTEMP = VTEMP * X0 **I_TMP1
                           DO WHILE (I_TMP1 .GT. 0 )
                              DNI1 = DNI1 * (I - I_TMP2) / (1 + I_TMP2)
                              I_TMP1 = I_TMP1 - 1
                              I_TMP2 = I_TMP2 + 1
                           END DO
                        ELSE IF (I_TMP1 .GT. 0) THEN    !X0==0
                           goto 7
                        END IF
                        I_TMP1 = J - J1
                        I_TMP2 = 0
                        IF (I_TMP1 .GT. 0 .and. Y0 .NE. 0.0D0) THEN
                           VTEMP = VTEMP * Y0 **I_TMP1
                           DO WHILE (I_TMP1 .GT. 0 )
                              DNI1 = DNI1 * (J - I_TMP2) / (1 + I_TMP2)
                              I_TMP1 = I_TMP1 - 1
                              I_TMP2 = I_TMP2 + 1
                           END DO
                        ELSE IF (I_TMP1 .GT. 0) THEN    !Y0==0
                           goto 7
                        END IF
                        I_TMP1 = K - K1
                        I_TMP2 = 0
                        IF (I_TMP1 .GT. 0 .and. Z0 .NE. 0.0D0) THEN
                           VTEMP = VTEMP * Z0 **I_TMP1
                           DO WHILE (I_TMP1 .GT. 0 )
                              DNI1 = DNI1 * (K - I_TMP2) / (1 + I_TMP2)
                              I_TMP1 = I_TMP1 - 1
                              I_TMP2 = I_TMP2 + 1
                           END DO
                        ELSE IF (I_TMP1 .GT. 0) THEN    !Z0==0
                           goto 7
                        END IF
                        ...
                        IF (J1 .GT. 0) VTEMP = VTEMP * Y1**J1
                        IF (K1 .GT. 0) VTEMP = VTEMP * Z1**K1
                        TCOEFF(IJK1) = TCOEFF(IJK1) + CTMP*VTEMP*DNI1
7                    CONTINUE
                  END DO
               END DO
6           CONTINUE
   ...
   SUBROUTINE POLY_GRAD (POS, VNOR)
      ...
      DO I = 0, NDEG
         DO J=0, NDEG
            DO 1 K=0, NDEG
               CTMP1 =  PCOEFF(I,J,K)
               if ( CTMP1 .EQ. 0.0D0 ) GOTO 1
               IF ((I.GT.1 .AND. X .EQ. 0D0) .OR. &
                  (J.GT.1 .AND. Y .EQ. 0D0) .OR.  &
                  (K.GT.1 .AND. Z .EQ. 0D0)) GOTO 1
               CTMP2 = CTMP1
               CTMP3 = CTMP1
! C      dF/dx
               IF (I .GT. 0 ) THEN
                  IF (I .GT. 1)  CTMP1 = I* CTMP1 * X**(I-1)
                  IF (J .GT. 0 )  CTMP1 =   CTMP1 * Y**J
                  IF (K .GT. 0 )  CTMP1 =   CTMP1 * Z**K
                  VNOR(1) = VNOR(1) + CTMP1
               END IF
! C      dF/dy
               IF ( J .GT. 0 ) THEN
                  IF (I .GT. 0 )  CTMP2 = CTMP2 * X**I
                  IF (J .GT. 1 )  CTMP2 = J*CTMP2 * Y**(J-1)
                  IF (K .GT. 0  ) CTMP2 = CTMP2 * Z**K
                  VNOR(2) = VNOR(2) + CTMP2
               END IF
! C      dF/dz
               IF ( K .GT. 0 ) THEN
                  IF (I .GT. 0 ) CTMP3 = CTMP3 * X**I
                  IF (J .GT. 0 ) CTMP3 = CTMP3 * Y**J
                  IF (K .GT. 1 ) CTMP3 = K*CTMP3 * Z**(K-1)
                  VNOR(3) = VNOR(3) + CTMP3
               END IF
1           CONTINUE
         END DO
...
! C---
   ...
   SUBROUTINE READPOLY (INFILE, IERR)
      ...
      DO 11 I = 0,MAX_NDEG    !4 to MAX_NDEG
         DO 21 J = 0,MAX_NDEG
            DO 31 K = 0,MAX_NDEG
               PCOEFF(I,J,K)	= 0.0D0
31          CONTINUE
...
11    CONTINUE
      ...
      IF (NDEG.GT.MAX_NDEG) CALL LEAVE &
         ('READPOLY','POLYNOMIAL NDEG TOO LARGE',NDEG)
      IF (NDEG .GT. 4) CALL MP_INIT()
      I = 0
41    IF (I.GE.0) THEN
...
300	  CONTINUE
            ...
            IF (F_R_RAN.EQ.1) THEN
               DO 1000 I=1,N_RIP
                  ...
                  AMPLI(I)	=   AMPL_IN(I)  !*WRAN(IG_SEED)  by YXJ
...
! C---
   SUBROUTINE REFLEC (PIN,WNUM,SIN_REF,COS_POLE,R_P,R_S,PHASEP,PHASES,ABSOR,K_WHAT)
      ...
      real(kind=skr)   :: ab_coeff, cos_ref, del_x, depth0, elfactor, gfact,SIN_REF2
      real(kind=skr)   :: qmin, qmax, qstep, ratio, phot_ener, ratio1, ratio2,ALFA
      real(kind=skr)   :: rho, rs1, rs2, tfact, tfilm, wnum0, xin, xlam, yin, gamma1
      integer(kind=ski):: i,j,nrefl,ierr,ier,index1,iunit,n_s,n_e,n_i
      integer(kind=ski):: ngx, ngy, ntx, nty, nin, npair
      complex(kind=skx)   :: nk2,rs,rp
...
! C
      ...
      IF (K_WHAT.EQ.1) THEN
         IF (F_REFL.NE.2) THEN
            ...
            ! R_P  =   SQRT(R_P)
            ! end SSLS:YXJ------------------------------------
            nk2 = CMPLX(1d0-ALFA,gamma1)
            SIN_REF2 = COS_REF*COS_REF
            Rs = ( SIN_REF -sqrt( nk2 - SIN_REF2 ) )/( SIN_REF + sqrt( nk2 - SIN_REF2 ) )
            Rp = ( nk2 * SIN_REF -sqrt( nk2 - SIN_REF2 ) )/( nk2*SIN_REF + sqrt( nk2 - SIN_REF2 ) )
            call atan_2(dimag(Rs), dreal(Rs),phases)
            call atan_2(dimag(Rp), dreal(Rp),phasep)
            R_S = cdabs(Rs)
            R_P = cdabs(Rp)
         ELSE
            ! C
            ...
            if (ENER(1) .lt. 100d0) then   !YXJ:SSLS, for energey below 102 eV
               if (PHOT_ENER .le. ENER(1))  then
                  index1 = 1
               else if (PHOT_ENER .ge. ENER(nin))  then
                  index1 = nin-1
               else
                  n_s = 1
                  n_e = nin
                  n_i = (n_s+n_e)/2
                  do while(.true.)
                     if (ener(n_i) .lt. PHOT_ENER) then
                        n_s = n_i
                     else if (ener(n_i) .gt. PHOT_ENER) then
                        n_e = n_i
                     end if
                     n_i = (n_s+n_e)/2
                     if ( n_i .eq. n_s ) then
...
! ** ref. frame for this case.
      ...
      ELSE IF (FMIRR.EQ.9) THEN
         IF ( .TRUE. ) THEN  ! NDEG .LE. 4
            CALL  POLY_GRAD (PIN,VOUT)
         ELSE
            CALL  MP_POLY_GRAD (PIN,VOUT)
         END IF
      END IF
      RETURN
...
!        real(kind=skr),dimension(12,NPOINT) :: out,ph_out,ap_out
      real(kind=skr),dimension(:,:),allocatable :: out,ph_out,ap_out
      ...
      allocate(out(12,NPOINT),stat=ierr)
      if ( ierr /= 0 ) then
         print *, "SCREEN: Allocate out failure, stop!"; STOP __LINE__
      end if
      allocate(ph_out(12,NPOINT),stat=ierr)
      if ( ierr /= 0 ) then
         print *, "SCREEN: Allocate ph_out failure, stop!"; STOP __LINE__
      end if
      allocate(ap_out(12,NPOINT),stat=ierr)
      if ( ierr /= 0 ) then
         print *, "SCREEN: Allocate ap_out failure, stop!"; STOP __LINE__
      end if
...
100     CONTINUE
      ...
      IF (I_ABS(I_WHAT).EQ.1) THEN
         ...
         FILE_REFL = FILE_TMP
      END IF
      deallocate(out);deallocate(ph_out);deallocate(ap_out)
...
! C
      IF (F_CRYSTAL.EQ.1) THEN
         ...
         RULING = 2d0*SIN(THETA_B-A_BRAGG)*SIN(ABS(A_BRAGG))/(TOCM/PHOT_CENT) !Xiaojiang: SSLS
...
! C SPLERR to keep track of errors in spline calulations.
      ...
      ELSE    ! Gaussian case
         ...
         CORR = 0.0D0
         CORR =  AMPLI(1)* SQRT(-2*LOG(WRAN(IG_SEED)))*COS(TWOPI*WRAN(IG_SEED))  !Xiaojiang: SSLS
         !DO I=1,N_RIP
         !CORR =   CORR  &
      ...
      ELSE
         ...
         V_CORR(2) = 0.0D0
         V_CORR(3) = 0.0D0
         V_CORR(1)  = SIG_X(1)*SQRT(-2*LOG(WRAN(IG_SEED)))*COS(TWOPI*WRAN(IG_SEED))
         V_CORR(2)  = SIG_Y(1)*SQRT(-2*LOG(WRAN(IG_SEED)))*COS(TWOPI*WRAN(IG_SEED))
...
!     	dimension	CODLING(12,npoint)
      real(kind=skr),dimension(:,:), allocatable :: CODLING  !(12,npoint)
      ...
      DIMENSION	AS_VEC(3),AS_TEMP(3),S_VEC(3),AS_ORG(3)
      DIMENSION	AP_VEC(3),AP_TEMP(3),AP_ORG(3)
...
!	dimension	ANGLE(4,npoint)
      real(kind=skr),dimension (:,:), allocatable :: ANGLE !(4,npoint)
      !INTEGER		SURFERR
      integer(kind=ski)  :: SURFERR,M_FLAG
#if DEFINED(MP_POLARIZATION) || defined(MP_A_NEW)
      !  mpfun variables
      type(mp_real) AS_VEC_MP(3),AS_TEMP_MP(3),AS_ORG_MP(3)
      type(mp_real) AP_VEC_MP(3),AP_TEMP_MP(3),AP_ORG_MP(3)
      type(mp_real) MP_TEMP1,MP_TEMP2, VVIN_MP(3),VNOR_MP(3)
      type(mp_real) A11_MP,A12_MP,A21_MP,A22_MP,PHS_MP,PHP_MP,AS_NEW_MP,AP_NEW_MP
      type(mp_real) PHTS_MP, PHBS_MP,PHTP_MP,PHBP_MP,Q_OUT_MP(3),MP_SMALL,MP_2PI
#endif
      complex(kind=skx)                ::nk1,nk2,Ts, Tp
!-------------------------------
...
!	SAVE		CODLING, ANGLE
      ...
      allocate(CODLING(12,npoint),stat=ierr)
      if ( ierr /= 0) then
         print *, "MIRROR1: Allocate CODLING error!" ; STOP __LINE__
      end if

      allocate(ANGLE(4,npoint),stat=ierr)
      if ( ierr /= 0) then
         print *, "MIRROR1: Allocate ANGLE error!" ; STOP __LINE__
      end if
...
! C
      ...
      END IF
#ifdef MP_POLARIZATION
!Xiaojiang: SSLS
      MP_2PI = 2*MP_PI()
      MP_SMALL = MPREALD(1D-40)
#endif
      XFIRST 	=   RWIDX2
      XSECON 	=   RWIDX1
...
! end if
         ...
         if ( (FMIRR .eq. 9) .and. ( (ITIK/1000)*1000 .eq. ITIK )) &
            print *, "tracing at ray number ", ITIK, ' of ', NPOINT
! C
...
! C	   	WRITE(6,*)'SURFERR = ',SURFERR
                  ...
                  RAY(10,ITIK) = SURFERR
                  !go to 301
                  K_1 = K_1 + 1     !Xiaojiang: SSLS
                  GO TO 10000      !Xiaojiang: SSLS, lost, no need to do trace
               END IF
! C
...
!if (itik.le.10) print *,'>>>0 alfa_old,alfa_new,new_obj,new_ima: ',alfa,rr_alfa,rr_ind_obj,rr_ind_ima
               ...
               !SSLS:YXJ, for CRL phase
               nk1 = cmplx(rr_ind_obj,-rr_attenuation_obj/Q_IN_MOD/2d0)
               nk2 = cmplx(rr_ind_ima,-rr_attenuation_ima/Q_IN_MOD/2d0)
               CALL DOT	(VVIN,VNOR,PHASES)
               CALL DOT	(VTEMP,VNOR,PHASEP)
               Ts = 2d0*nk1*PHASES/(nk1*phases + nk2*phasep)
               Tp = 2d0*nk1*PHASES/(nk1*PHASEP + nk2*phases)         
               call atan_2(DIMAG(Ts),DREAL(Ts),phases)
               call atan_2(DIMAG(Tp),DREAL(Tp),PHASEP)
               PHASE(2,ITIK) = PHASE(2,ITIK) + phases
               PHASE(3,ITIK) = PHASE(3,ITIK) + phasep
               !end SSLS:YXJ
...
!     	  RAY(6,ITIK)	=   VTEMP(3)
               ...
               if (isnan(SQRT (TTEMP)) ) then
                  ...
                  !IF (RAY(11,ITIK).NE.0.0D0)  RAY(11,ITIK)  =   SQRT (TTEMP)     !Xiaojiang: SSLS Not necessary
               end if
...
! C
               IF (ORDER.NE.0.AND.RAY(11,ITIK).NE.0) &
                  PHASE(1,ITIK)	= PHASE(1,ITIK) !+ PPOUT(2)*G_MOD/RAY(11,ITIK)  !SSLS: Xiaojiang
            ...
            ELSE IF (F_RULING.EQ.5) THEN
               ...
               PHASE(1,ITIK) = PHASE(1,ITIK) !+ TWOPI*ORDER*PPOUT(2)*( Ruling &
                  ...
                  !+ (DIST*user_units_to_cm)**3 * RUL_A3)/RAY(11,ITIK)  !SSLS: Xiaojiang, Bypass
                  ...
                  ELSE IF (FMIRR.EQ.3) THEN                   !Torus case
                     ...
                     if (I_JOHANSSON .EQ. 1 ) then   !Xiaojiang: SSLS
                        ...
                        if ( Z_ROT .ge. 0d0) then
                           ...
                           CALL	CROSS	(VNOR,X_VRS,VTEMP)  !Xiaojiang: SSLS
                           CALL CROSS (VNOR,VTEMP,VTAN)    !Xiaojiang: SSLS
                           ARC = (-TMP - ATAN(-PPOUT(1)/(R_JOHANSSON-R_MIN*(1-cos(TMP)))))*R_JOHANSSON   !Xiaojiang: SSLS
                        else
                           CALL	CROSS	(X_VRS,VNOR,VTEMP)   !Xiaojiang: SSLS
                           CALL CROSS (VNOR,VTEMP,VTAN)    !Xiaojiang: SSLS
                           ARC = (TMP - ATAN(-PPOUT(1)/(R_JOHANSSON-R_MIN*(1-cos(TMP)))))*R_JOHANSSON   !Xiaojiang: SSLS
                        end if
                     else
                        CALL	CROSS	(Y_VRS,VNOR,VTEMP)   !Xiaojiang: SSLS
                        CALL CROSS (VNOR,VTEMP,VTAN)     !Xiaojiang: SSLS
                        TMP = ASIN(PPOUT(2)/(R_MAJ+R_MIN))   
                        ARC = (TMP - ATAN(PPOUT(2)/(R_JOHANSSON-(R_MAJ+R_MIN)*(1-cos(TMP)))))*R_JOHANSSON   !Xiaojiang: SSLS
                     end if   !Xiaojiang: SSLS
                     CALL	NORM	(VTAN,VTAN)          !Xiaojiang: SSLS
...
#ifndef MP_POLARIZATION
         CALL CROSS_M_FLAG  (VVIN,VNOR,AS_TEMP,M_FLAG) ! vector pp. to inc.pl.
         CALL	DOT	(AS_VEC,AS_VEC,AS2)
         CALL	DOT	(AP_VEC,AP_VEC,AP2)
         IF (AS2 .GE.AP2) THEN   !USE MAJOR DIRECITON TO DEFINE ORIGINAL VECTORS
            DO I=1,3
               AS_ORG(I) = AS_VEC(I)
            END DO
            CALL NORM(AS_ORG,AS_ORG)   !YXJ:SSLS, avoid tiny value in cross product
            CALL CROSS (AS_ORG,VVIN,AP_ORG)
         ELSE
            DO I=1,3
               AP_ORG(I) = AP_VEC(I)
            END DO
            CALL NORM(AP_ORG,AP_ORG)   !YXJ:SSLS, avoid tiny value in cross product
            CALL CROSS (VVIN,AP_ORG,AS_ORG)
            !yxj: ---------------------------------------------------
         END IF
         IF (M_FLAG.EQ.1 ) THEN
            !	    CALL	DOT	(AS_VEC,AS_VEC,AS2)   !YXJ, SSLS
            !	    CALL	DOT	(AP_VEC,AP_VEC,AP2)   !YXJ, SSLS
            IF (AS2 .GT.AP2)	THEN
               DO 499 I=1,3
 499   	   AS_TEMP(I) = AS_VEC(I)
	ELSE
               DO 599 I=1,3
599            AP_TEMP(I) = AP_VEC(I)
               CALL	NORM	(AP_TEMP,AP_TEMP) !YXJ:SSLS, avoid tiny value in cross product
               CALL CROSS (VVIN,AP_TEMP,AS_TEMP)
            END IF
         END IF
         ...
         A22_MP   = MPREALD(A22)
! ** Now recompute the ampltitude and phase of the local S- and P- component.
         MP_TEMP1 = COS(PHS_MP-PHP_MP)
         AS_NEW_MP	= SQRT(ABS(A11_MP**2 + A12_MP**2 +  &
            2.0D0*A11_MP*A12_MP*MP_TEMP1))
         AP_NEW_MP	= SQRT(ABS(A21_MP**2 + A22_MP**2 +  &
            2.0D0*A21_MP*A22_MP*MP_TEMP1))
         AS_NEW = AS_NEW_MP
         AP_NEW = AP_NEW_MP
#else      
...
#else    !Xiaojiang: SSLS
   !C:\yuxj\shadow3-python\src\mpfun2020\mpfunf.f90, precision control mpiplm
         DO I=1,3
            VVIN_MP(I)   = MPREALD(VVIN(I))
            VNOR_MP(I)   = MPREALD(VNOR(I))
            AS_VEC_MP(I) = MPREALD(AS_VEC(I))
            AP_VEC_MP(I) = MPREALD(AP_VEC(I))
         END DO
         CALL CROSS_MP  (VVIN_MP,VNOR_MP,AS_TEMP_MP,M_FLAG) ! vector pp. to inc.pl.
!---------------YXJ, SSLS--------------------
         CALL	DOT	(AS_VEC,AS_VEC,AS2)
         CALL	DOT	(AP_VEC,AP_VEC,AP2)
         IF (AS2 .GT.AP2) THEN   !USE MAJOR DIRECITON TO DEFINE ORIGINAL VECTORS
            DO I=1,3
               AS_ORG_MP(I) = AS_VEC_MP(I)
            END DO
            CALL NORM_MP(AS_ORG_MP,AS_ORG_MP)   !YXJ:SSLS, avoid tiny value in cross product
            CALL CROSS_MP (AS_ORG_MP,VVIN_MP,AP_ORG_MP)
         ELSE
            DO I=1,3
               AP_ORG_MP(I) = AP_VEC_MP(I)
            END DO
            CALL NORM_MP(AP_ORG_MP,AP_ORG_MP)   !YXJ:SSLS, avoid tiny value in cross product
            CALL CROSS_MP (VVIN_MP,AP_ORG_MP,AS_ORG_MP)
!yxj: ---------------------------------------------------
         END IF
         IF (M_FLAG.EQ.1 ) THEN
!	    CALL	DOT	(AS_VEC,AS_VEC,AS2)   !YXJ, SSLS
!	    CALL	DOT	(AP_VEC,AP_VEC,AP2)   !YXJ, SSLS
            IF (AS2 .GT.AP2)	THEN
               DO 499 I=1,3
499            AS_TEMP_MP(I) = AS_VEC_MP(I)
            ELSE       !YXJ
               DO 599 I=1,3
599            AP_TEMP_MP(I) = AP_VEC_MP(I)
               CALL	NORM_MP	(AP_TEMP_MP,AP_TEMP_MP) !YXJ:SSLS, avoid tiny value in cross product
               CALL CROSS_MP (VVIN_MP,AP_TEMP_MP,AS_TEMP_MP)
            END IF
         END IF
         CALL	NORM_MP  (AS_TEMP_MP,AS_TEMP_MP)	! Local unit As vector
         CALL	CROSS_MP	(AS_TEMP_MP,VVIN_MP,AP_TEMP_MP)
         CALL	NORM_MP	(AP_TEMP_MP,AP_TEMP_MP)	! Local unit Ap vector
         CALL	DOT_MP	(AS_VEC_MP,AS_TEMP_MP,A11_MP)	! matrix element of rotation
         CALL	DOT_MP	(AP_VEC_MP,AS_TEMP_MP,A12_MP)	! matrix element of rotation
         CALL	DOT_MP	(AS_VEC_MP,AP_TEMP_MP,A21_MP)	! matrix element of rotation
         CALL	DOT_MP	(AP_VEC_MP,AP_TEMP_MP,A22_MP)	! matrix element of rotation
         PHS	= PHASE(2,ITIK)
         PHP	= PHASE(3,ITIK)
         PHS_MP	= MPREALD(PHS)
         PHP_MP	= MPREALD(PHP)
! ** Now recompute the ampltitude and phase of the local S- and P- component.
         ...
         AP_NEW_MP	= SQRT(ABS(A21_MP**2 + A22_MP**2 +  &
            2.0D0*A21_MP*A22_MP*MP_TEMP1))
         CALL	SCALAR_MP	(AS_TEMP_MP,AS_NEW_MP,AS_VEC_MP)	! Local As vector
         CALL	SCALAR_MP	(AP_TEMP_MP,AP_NEW_MP,AP_VEC_MP)	! Local Ap vector
         PHTS_MP	= A11_MP*SIN(PHS_MP) + A12_MP*SIN(PHP_MP)
         PHBS_MP	= A11_MP*COS(PHS_MP) + A12_MP*COS(PHP_MP)
         PHTP_MP	= A21_MP*SIN(PHS_MP) + A22_MP*SIN(PHP_MP)
         PHBP_MP	= A21_MP*COS(PHS_MP) + A22_MP*COS(PHP_MP)
         PHS_MP	= ATAN2	(PHTS_MP,PHBS_MP)		! Phase of local As vector
         PHP_MP	= ATAN2	(PHTP_MP,PHBP_MP)		! Phase of local Ap vector
! C
! C
         CALL  SCALAR_MP   (Q_OUT,1d0,Q_OUT_MP)
         CALL	DOT_MP	(VVIN_MP,VNOR_MP,MP_TEMP1)	! sin(graz. ang)
         CALL	DOT_MP	(Q_OUT_MP,VNOR_MP,MP_TEMP2)	! sin(graz.ref.ang)
         SIN_VAL = MP_TEMP1
         SIN_REF = MP_TEMP2
#endif
! C
...
#else
               mp_temp1 = MPREALD(REF_S)
               mp_temp2 = MPREALD(REF_P)
               CALL	SCALAR_MP  (AS_VEC_MP,mp_temp1,AS_VEC_MP)
               CALL	SCALAR_MP  (AP_VEC_MP,MP_TEMP2,AP_VEC_MP)
               PHP_MP	= PHP_MP + mpreald(PHASEP)
               PHS_MP	= PHS_MP + mpreald(PHASES)
               EFF_REF_S = EFF_REF_S + REF_S**2
               EFF_REF_P = EFF_REF_P + REF_P**2
            ELSE IF (F_REFLEC.EQ.2) THEN			! No interested in it
               REF		= (REF_S**2 + REF_P**2)/2
               EFF_REF 	= EFF_REF + REF
               REF		= SQRT(REF)
               mp_temp1 = MPREALD(REF)
               CALL	SCALAR_MP	(AP_VEC_MP,mp_temp1,AP_VEC_MP)
               CALL	SCALAR_MP	(AS_VEC_MP,mp_temp1,AS_VEC_MP)
#endif
            END IF
         END IF
...
! C
         ...
         !PHS	= PHS - PI       !YXJ, phase changed, but AS_, AP_VEC not changed due to code from 9076
         ...
         !END IF
! YXJ:SSLS Round the phase in between [-Pi, +Pi]   by adding 2pi*n
#ifndef MP_POLARIZATION
         iTemp = ceiling(abs(PHS)/pi)/2
         if (PHS .lt. 0 ) iTemp = -iTemp
         if (iTemp .ne. 0) PHS = PHS - iTemp * twopi
         iTemp = ceiling(abs(PHP)/pi)/2
         if (PHP .lt. 0 ) iTemp = -iTemp
         if (iTemp .ne. 0) PHP = PHP - iTemp * twopi
#else
         iTemp = ceiling(abs(PHS_MP)/MP_PI)/2
         if (PHS_MP .lt. 0 ) iTemp = -iTemp
         if (iTemp .ne. 0) PHS_MP = PHS_MP - iTemp * MP_2PI
         iTemp = ceiling(abs(PHP_MP)/MP_TEMP1)/2
         if (PHP_MP .lt. 0 ) iTemp = -iTemp
         if (iTemp .ne. 0) PHP_MP = PHP_MP - iTemp * MP_2PI
#endif
...
! new ones do not work as it may be a component of the electric field along VVOUT
         ...
         !---------------YXJ, SSLS--------------------
#ifndef USE_MP_PHASE
         CALL	DOT	(AS_VEC,AS_ORG,A11)	! matrix element of rotation
         CALL	DOT	(AP_VEC,AS_ORG,A12)	! matrix element of rotation
         CALL	DOT	(AS_VEC,AP_ORG,A21)	! matrix element of rotation
         CALL	DOT	(AP_VEC,AP_ORG,A22)	! matrix element of rotation

         ! project the ampltitude and phase of the local S- and P- component back to original direction.
         ! PHS, PHP already calculated above
         AS_NEW	= SQRT(ABS(A11**2 + A12**2 +  &
            2.0D0*A11*A12*COS(PHS-PHP)))
         AP_NEW	= SQRT(ABS(A21**2 + A22**2 +  &
            2.0D0*A21*A22*COS(PHS-PHP)))
! XIAOJIANG (SSLS): ALIGN POLARIZATION VECTOR WITH OUTGOING DIRECTION
         CALL CROSS_M_FLAG(VVIN,VVOUT,S_VEC,M_FLAG)
         IF (M_FLAG .NE. 1) THEN
            CALL NORM(S_VEC,S_VEC)
            CALL DOT(VVIN,VVOUT,TEMP)
            IF ( TEMP .GT. 1D0) TEMP =1D0
            IF ( TEMP .LT. -1D0) TEMP =-1D0
            ROT_S_ANG = -ACOS(TEMP)
            CALL ROTVECTOR(AS_ORG,S_VEC,ROT_S_ANG,AS_ORG)
            CALL ROTVECTOR(AP_ORG,S_VEC,ROT_S_ANG,AP_ORG)
            CALL NORM(AS_ORG,AS_ORG)
            CALL NORM(AP_ORG,AP_ORG)
         END IF
!
         CALL	SCALAR	(AS_ORG,AS_NEW,AS_VEC)	! Original As vector
         CALL	SCALAR	(AP_ORG,AP_NEW,AP_VEC)	! original Ap vector
         PHTS	= A11*SIN(PHS) + A12*SIN(PHP)
         PHBS	= A11*COS(PHS) + A12*COS(PHP)
         PHTP	= A21*SIN(PHS) + A22*SIN(PHP)
         PHBP	= A21*COS(PHS) + A22*COS(PHP)
         ...
         CALL	ATAN_2	(PHTP,PHBP,PHP)		! Phase of local Ap vector
#else
         CALL	DOT_MP	(AS_VEC_MP,AS_ORG_MP,A11_MP)	! matrix element of rotation
         CALL	DOT_MP	(AP_VEC_MP,AS_ORG_MP,A12_MP)	! matrix element of rotation
         CALL	DOT_MP	(AS_VEC_MP,AP_ORG_MP,A21_MP)	! matrix element of rotation
         CALL	DOT_MP	(AP_VEC_MP,AP_ORG_MP,A22_MP)	! matrix element of rotation
         ...
         ! CALL	SCALAR_MP	(AS_ORG_MP,AS_NEW_MP,AS_VEC)	! Original As vector
         ! CALL	SCALAR_MP	(AP_ORG_MP,AP_NEW_MP,AP_VEC)	! original Ap vector
         PHTS_MP	= A11_MP*SIN(PHS_MP) + A12_MP*SIN(PHP_MP)
         PHBS_MP	= A11_MP*COS(PHS_MP) + A12_MP*COS(PHP_MP)
         PHTP_MP	= A21_MP*SIN(PHS_MP) + A22_MP*SIN(PHP_MP)
         PHBP_MP	= A21_MP*COS(PHS_MP) + A22_MP*COS(PHP_MP)
         IF (ABS(PHTS_MP).LT.MP_SMALL ) PHTS_MP = MPREALD(0D0)
         IF (ABS(PHTP_MP).LT.MP_SMALL)  PHTP_MP = MPREALD(0D0)
         PHS =	ATAN2	(PHTS_MP,PHBS_MP)		! Phase of local As vector
         PHP = ATAN2	(PHTP_MP,PHBP_MP)		! Phase of local Ap vector
#endif
...
! C write incidence and reflection information to file
            ...
            DO 2525 J = 1,NPOINT
2525        WRITE(55,'(1X,I10,2F20.15,I10)') int(ANGLE(1,J)),ANGLE(2,J),ANGLE(3,J),int(ANGLE(4,J))
            CLOSE(55)
...
!  SUBROUTINE 	SOURCE1B (infile, FNAME, IOFORM)
   SUBROUTINE sourceGeom (pool00,ray,npoint1) !bind(C,NAME="sourceGeom")
      ...
      integer(kind=ski), intent(in out)    :: npoint1
      ...
      !pool00%NPOINT = npoint1
      !NTOTAL = npoint1
      allocate(grid(6,npoint1),stat=ierr)
      IF (ierr /= 0) THEN
         print *,"sourceGeom: Error allocating GRID" ; STOP 11117
      END IF
      n_rej=0
      k_rej=0
      xxx=0
...
41       CONTINUE
      ...
      ELSE IF (FGRID.EQ.1.OR.FGRID.EQ.3) THEN
         ...
         DO 51 C_VX = -CL_VX, CL_VX+0.5D0
            !!srio  DO 61 C_VZ = -CL_VZ, CL_VZ
            DO 61 C_VZ = -CL_VZ, CL_VZ+0.5d0
...
113      CONTINUE
      ELSE IF (FGRID.EQ.1.OR.FGRID.EQ.2) THEN
         ...
         DO 131 C_X = -CL_X, CL_X + 0.5D0
            DO 141 C_Y = -CL_Y, CL_Y + 0.5D0
               DO 151 C_Z = -CL_Z, CL_Z + 0.5d0
                  DO 161 J = 1, INDEXMOM
                     INDEXSPA = INDEXSPA + 1
...
11       CONTINUE
         ...
         ZMAX1		=   VDIV1
         ZMAX2		=  -VDIV2
         XRAND 	= (GRID(4,ITIK)*(XMAX1 - XMAX2) + XMAX2)  ! phi
         ZRAND 	= (GRID(6,ITIK)*(ZMAX1 - ZMAX2) + ZMAX2)  ! psi
!C	Xiaojiang: replace with actual directioal consine
! DIREC(1) 	= SIN(XRAND) * COS(ZRAND)   		            ! X	   l
! DIREC(2) 	= COS(XRAND) * COS(ZRAND)         		      ! Y	   m
! DIREC(3) 	= SIN(ZRAND)         		  		            ! Z	   n
         DIREC(1) 	= SIN(XRAND)   		                        ! X	   l
         DIREC(2) 	= SQRT(COS(ZRAND)**2- SIN(XRAND)**2)	      ! Y	   m
         DIREC(3) 	= SIN(ZRAND)                                 ! Z	   n
!C   error when all angle is zero
!    commenout by Dr. XJ.Yu
!       XMAX1  =   TAN(HDIV1)
!       XMAX2 = - TAN(HDIV2)
!       ZMAX1 =   TAN(VDIV1)
!       ZMAX2 = - TAN(VDIV2)
...
!      following already comment out orginally
...
!only z axis direction (vertival direction in source frame), electrical field directions decide later
         AP_VEC(1) =   0.0D0
         AP_VEC(2) =   0.0D0
         AP_VEC(3) =   1.0D0
         ! C
         ...
         ! C
         CALL CROSS (DIREC,AP_VEC,A_VEC)
         CALL CROSS (A_VEC,DIREC,AP_VEC)
         CALL NORM (A_VEC,A_VEC)
         CALL NORM (AP_VEC,AP_VEC)

         IF (F_POLAR.EQ.1 .and. POL_DEG .ge.0 ) THEN
            ! C
...
! C---
   SUBROUTINE RESTART18 (RAY18,NCOL1,NPOINT1) !,RAY,PHASE,AP)
      ...
      real(kind=skr),dimension(:,:),allocatable     :: PHASE, AP
      integer(kind=ski) :: ierr,i,j
!
!       initialize
!
      allocate(RAY(NCOL1,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"RESTART18: Allocate RAY failure. Stop!";STOP __LINE__
      end if
      allocate(PHASE(3,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"RESTART18: Allocate PHASE failure. Stop!";STOP __LINE__
      end if
      allocate(AP(3,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"RESTART18: Allocate AP failure. Stop!";STOP __LINE__
      end if
! create a tempo array with value 0, then copy to RAY, etc
! if the array is very huge, will cause stack overflow
! RAY   = 0.0D0
      DO I=1,NPOINT1
         DO J=1,NCOL1
            RAY(J,I)   = RAY18(J,I)
...
!
      ...
      DO I=1,NPOINT1
         ...
         IF (NCOL1 == 18) THEN
            do j=1,3
               RAY18(12+j,i) = PHASE(j,i)
               RAY18(15+j,i) = AP(j,i)
            enddo
         END IF
      enddo
      deallocate(RAY); deallocate(phase); deallocate(AP)
...
!
      allocate(RAY(NCOL1,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"SCREEN18: Allocate RAY failure. Stop!";STOP __LINE__
      end if
      allocate(PHASE(3,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"SCREEN18: Allocate PHASE failure. Stop!";STOP __LINE__
      end if
      allocate(AP(3,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"SCREEN18: Allocate AP failure. Stop!";STOP __LINE__
      end if
! create a tempo array with value 0, then copy to RAY, etc
! if the array is very huge, will cause stack overflow
! RAY   = 0.0D0
...
!        real(kind=skr),dimension(3,NPOINT1)     :: PHASE, AP
      ...
      allocate(RAY(NCOL1,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"MIRROR18: Allocate RAY failure. Stop!";STOP __LINE__
      end if
      allocate(PHASE(3,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"MIRROR18: Allocate PHASE failure. Stop!";STOP __LINE__
      end if
      allocate(AP(3,NPOINT1),stat=ierr)
      if ( ierr /= 0 ) then
         print *,"MIRROR18: Allocate AP failure. Stop!";STOP __LINE__
      end if
! create a tempo array with value 0, then copy to RAY, etc
! if the array is very huge, will cause stack overflow
!
...
!NEW MP VERSION FOR POLY
   SUBROUTINE MP_POLY (XIN, VIN, ANSWER, I_RES)
      real*8 XIN(3), VIN(3), ANSWER
      INTEGER  I_RES, N_TEST, IER
      real*8 h_output88(MAX_NDEG*2)
      complex*16 H_OUTPUT(MAX_NDEG)
      equivalence (h_output88(1), H_OUTPUT(1))

      real*8 TEST1(MAX_NDEG), TEST2(MAX_NDEG), CHECK, TEST
      REAL COEFF(21,MAX_NDEG+1), TCOEFF(21,MAX_NDEG+1)
      INTEGER  i, j, k, i1, j1, k1, IJK1, I_TMP1, I_TMP2, MOVE
      REAL X0(21), Y0(21), Z0(21), X1(21),Y1(21),Z1(21),CTMP(21)
      LOGICAL MPL1,MPL2,MPL3
      REAL MPM1(21), VTEMP(21), DNI1(21), MPM2(21)
      INTEGER MPI1
      DOUBLE PRECISION MPD1
      character*1 css(100)

      ANSWER = 0.0D0
      CALL MPDMC (XIN(1), 0, X0)
      CALL MPDMC (XIN(2), 0, Y0)
      CALL MPDMC (XIN(3), 0, Z0)
      CALL MPDMC (VIN(1), 0, X1)
      CALL MPDMC (VIN(2), 0, Y1)
      CALL MPDMC (VIN(3), 0, Z1)
      DO I = 1, MAX_NDEG+1
         CALL MPDMC (0.0D0, 0, TCOEFF(1,I))
      END DO
      DO I=NDEG,0,-1
         DO J=0,NDEG
            IF ( (I+J) .GT. NDEG ) exit
            DO 6 K=0, NDEG
               CALL MPDMC (PCOEFF(I,J,K), 0, CTMP)
               IF ( (I+J+K) .GT. NDEG) exit
               CALL MPDMC (0.0D0, 0, MPM1)
               CALL MPCPR (CTMP, MPM1, MPI1)
               MPL1 = MPI1 .EQ. 0
               IF (MPL1) THEN
                  GOTO 6
               ENDIF
               DO I1=0,I
                  DO J1=0,J
                     DO 7 K1=0,K
                        CALL MPDMC (1.0D0, 0, VTEMP)
                        CALL MPDMC (1.0D0, 0, DNI1)
                        I_TMP1 = I - I1
                        I_TMP2 = 0
                        MPL1 = I_TMP1 .GT. 0
                        CALL MPDMC (0.0D0, 0, MPM1)
                        CALL MPCPR (X0, MPM1, MPI1)
                        MPL2 = MPI1 .NE. 0
                        MPL3 = MPL1 .AND. MPL2
                        IF (MPL3) THEN
!   CMP<
!   CMP>                                 VTEMP = VTEMP * X0 **I_TMP1
                           CALL MPNPWR (X0, I_TMP1, MPM1)
                           CALL MPMUL (VTEMP, MPM1, MPM2)
                           CALL MPEQ (MPM2, VTEMP)
!   CMP<
101                        CONTINUE
                           IF (I_TMP1 .EQ. 0 ) GO TO 102
!   CMP>                          DNI1 = DNI1 * (I - I_TMP2) / (1 + I_TMP2)
                           MPI1 = I - I_TMP2
                           MPD1 = MPI1
                           CALL MPMULD (DNI1, MPD1, 0, MPM1)
                           MPI1 = 1 + I_TMP2
                           MPD1 = MPI1
                           CALL MPDIVD (MPM1, MPD1, 0, MPM2)
                           CALL MPEQ (MPM2, DNI1)
!   CMP<
                           I_TMP1 = I_TMP1 - 1
                           I_TMP2 = I_TMP2 + 1
                           GO TO 101
102                        CONTINUE
                        ELSE IF (I_TMP1 .GT. 0) THEN
                           goto 7
                        END IF
                        I_TMP1 = J - J1
                        I_TMP2 = 0
!   CMP>                          IF (I_TMP1 .GT. 0 .and. Y0 .NE. 0.0D0) THEN
                        MPL1 = I_TMP1 .GT. 0
                        CALL MPDMC (0.0D0, 0, MPM1)
                        CALL MPCPR (Y0, MPM1, MPI1)
                        MPL2 = MPI1 .NE. 0
                        MPL3 = MPL1 .AND. MPL2
                        IF (MPL3) THEN
!   CMP<
!   CMP>                                 VTEMP = VTEMP * Y0 **I_TMP1
                           CALL MPNPWR (Y0, I_TMP1, MPM1)
                           CALL MPMUL (VTEMP, MPM1, MPM2)
                           CALL MPEQ (MPM2, VTEMP)
!   CMP<
103                        CONTINUE
                           IF (I_TMP1 .EQ. 0 ) GO TO 104
!   CMP>                          DNI1 = DNI1 * (J - I_TMP2) / (1 + I_TMP2)
                           MPI1 = J - I_TMP2
                           MPD1 = MPI1
                           CALL MPMULD (DNI1, MPD1, 0, MPM1)
                           MPI1 = 1 + I_TMP2
...
104                        CONTINUE
                        ...
                        I_TMP1 = K - K1
                        I_TMP2 = 0
!   CMP>                          IF (I_TMP1 .GT. 0 .and. Z0 .NE. 0.0D0) THEN
                        MPL1 = I_TMP1 .GT. 0
                        CALL MPDMC (0.0D0, 0, MPM1)
                        CALL MPCPR (Z0, MPM1, MPI1)
                        MPL2 = MPI1 .NE. 0
                        MPL3 = MPL1 .AND. MPL2
                        IF (MPL3) THEN
!   CMP<
!   CMP>                                 VTEMP = VTEMP * Z0 **I_TMP1
                           CALL MPNPWR (Z0, I_TMP1, MPM1)
                           CALL MPMUL (VTEMP, MPM1, MPM2)
                           CALL MPEQ (MPM2, VTEMP)
!   CMP<
105                        CONTINUE
                           IF (I_TMP1 .EQ. 0 ) GO TO 106
!   CMP>                          DNI1 = DNI1 * (K - I_TMP2) / (1 + I_TMP2)
                           MPI1 = K - I_TMP2
                           MPD1 = MPI1
                           CALL MPMULD (DNI1, MPD1, 0, MPM1)
                           MPI1 = 1 + I_TMP2
...
106                        CONTINUE
                        ...
                        IJK1 = I1 + J1 + K1 + 1
!   CMP>                          IF (I1 .GT. 0) VTEMP = VTEMP * X1**I1
                        IF (I1 .GT. 0)  THEN
                           CALL MPNPWR (X1, I1, MPM1)
                           CALL MPMUL (VTEMP, MPM1, MPM2)
                           CALL MPEQ (MPM2, VTEMP)
                        ENDIF
!   CMP<
!   CMP>                          IF (J1 .GT. 0) VTEMP = VTEMP * Y1**J1
                        IF (J1 .GT. 0) THEN
                           CALL MPNPWR (Y1, J1, MPM1)
                           CALL MPMUL (VTEMP, MPM1, MPM2)
                           CALL MPEQ (MPM2, VTEMP)
                        ENDIF
!   CMP<
!   CMP>                          IF (K1 .GT. 0) VTEMP = VTEMP * Z1**K1
                        IF (K1 .GT. 0) THEN
                           CALL MPNPWR (Z1, K1, MPM1)
                           CALL MPMUL (VTEMP, MPM1, MPM2)
                           CALL MPEQ (MPM2, VTEMP)
                        ENDIF
!   CMP<
!   CMP>                   TCOEFF(IJK1) = TCOEFF(IJK1) + CTMP*VTEMP*DNI1
                        CALL MPMUL (CTMP, VTEMP, MPM1)
                        CALL MPMUL (MPM1, DNI1, MPM2)
                        CALL MPADD (TCOEFF(1,IJK1), MPM2, MPM1)
                        CALL MPEQ (MPM1, TCOEFF(1,IJK1))
!   CMP<
7                    CONTINUE
                  END DO
               END DO
...
!   C
      DO 11 I=NDEG+1,1,-1
!   CMP>            COEFF(NDEG-I+2) = TCOEFF(I)
         CALL MPEQ (TCOEFF(1,I), COEFF(1,NDEG-I+2))
!   CMP<
11    CONTINUE
!   CMP>     CALL MP_ZRPOLY (COEFF,NDEG,H_OUTPUT88,IER)
      CALL MP_ZRPOLY (COEFF, NDEG, H_OUTPUT88, IER)
!   CMP<
      IF (IER.EQ.130) THEN
         MOVE = 0
21       IF (IER.NE.0) THEN
            MOVE = MOVE + 1
            IF (MOVE.EQ.4) THEN
               PRINT *,'POLY','ERROR IN POLY: ',izero
               STOP
            END IF
            DO 31 I=1,NDEG
!   CMP>                   TCOEFF(I) = COEFF(I+1)
               CALL MPEQ (COEFF(1,I+1), TCOEFF(1,I))
!   CMP<
31          CONTINUE
            DO  I=1,NDEG
!   CMP>                   COEFF(I)=TCOEFF(I)
               CALL MPEQ (TCOEFF(1,I), COEFF(1,I))
!   CMP<
            END DO
!   CMP>            CALL MP_ZRPOLY (COEFF,NDEG,H_OUTPUT88,IER)
            CALL MP_ZRPOLY (COEFF, NDEG, H_OUTPUT88, IER)
!   CMP<
         ...
         END IF
      ELSE IF (IER.EQ.131) THEN
         PRINT *,'POLY','Fatal error: ',izero
         STOP
      END IF
      CHECK = 1
      DO 51 I=1,NDEG
         IF ( ABS(H_OUTPUT88(I+1)) .LT. 1.0E-14 ) THEN
            CHECK = 0
            EXIT
         END IF
51    CONTINUE
      ...
      IF (I_RES.LT.0) THEN
         ANSWER =1.0D30
         DO 61 I=1,NDEG
            TEST = ABS (H_OUTPUT88(I+1))
            IF (TEST.LT.1.0E-14) THEN
               IF (ABS(H_OUTPUT88(I)).LT.ABS(ANSWER)) &
                  ANSWER = H_OUTPUT88(I)
            END IF
61       CONTINUE
      ELSE
...
71       CONTINUE
      ...
      END IF
      RETURN
   End SUBROUTINE MP_POLY

   SUBROUTINE MP_POLY_GRAD (POS, VNOR)
      real*8 POS(3), VNOR(3)
      REAL MP_VNOR(21,3)
      REAL*8 X0, Y0, Z0, MPD1
      INTEGER  I, J, K, MPI1

      REAL X(21), Y(21), Z(21), CTMP1(21), MPM1(21),CTMP2(21),CTMP3(21)
      REAL MPM2(21), MPM3(21)
      LOGICAL MPL1
...
!   CMP<
      DO I=1,3
!   CMP>            MP_VNOR(I) = 0.0D0
         CALL MPDMC (0.0D0, 0, MP_VNOR(1,I))
!   CMP<
      END DO
      DO I = 0, NDEG
         DO J=0, NDEG
            DO 1 K=0, NDEG
!   CMP>                   CTMP1 =  PCOEFF(I,J,K)
               CALL MPDMC (PCOEFF(I,J,K), 0, CTMP1)
!   CMP<
!   CMP>                   if ( CTMP1 .EQ. 0.0D0 ) GOTO 1
               CALL MPDMC (0.0D0, 0, MPM1)
               CALL MPCPR (CTMP1, MPM1, MPI1)
               MPL1 = MPI1 .EQ. 0
               IF (MPL1) THEN
                  GOTO 1
               ENDIF
!   CMP<
               IF ( (I.GT.1 .AND. X0 .EQ. 0D0) .OR.  &
                  (J.GT.1 .AND. Y0 .EQ. 0D0) .OR.    &
                  (K.GT.1 .AND. Z0 .EQ. 0D0)) GOTO 1
!   CMP>                   CTMP2 = CTMP1
               CALL MPEQ (CTMP1, CTMP2)
!   CMP<
!   CMP>                   CTMP3 = CTMP1
               CALL MPEQ (CTMP1, CTMP3)
!   CMP<
!   C      dF/dx
               IF (I .GT. 0 ) THEN
!   CMP>                          IF (I .GT. 1)
!   CMP>   $                       CTMP1 = I* CTMP1 * X**(I-1)
                  IF (I .GT. 1) THEN
                     MPD1 = I
                     CALL MPMULD (CTMP1, MPD1, 0, MPM1)
                     MPI1 = I - 1
                     CALL MPNPWR (X, MPI1, MPM2)
                     CALL MPMUL (MPM1, MPM2, MPM3)
                     CALL MPEQ (MPM3, CTMP1)
                  ENDIF
!   CMP<
!   CMP>                          IF (J .GT. 0 )
!   CMP>   $                       CTMP1 = CTMP1 * Y**J
                  IF (J .GT. 0 ) THEN
                     CALL MPNPWR (Y, J, MPM1)
                     CALL MPMUL (CTMP1, MPM1, MPM2)
                     CALL MPEQ (MPM2, CTMP1)
                  ENDIF
!   CMP<
!   CMP>                          IF (K .GT. 0 )
!   CMP>   $                       CTMP1 = CTMP1 * Z**K
                  IF (K .GT. 0 )  THEN
                     CALL MPNPWR (Z, K, MPM1)
                     CALL MPMUL (CTMP1, MPM1, MPM2)
                     CALL MPEQ (MPM2, CTMP1)
                  ENDIF
!   CMP<
!   CMP>                          MP_VNOR(1) = MP_VNOR(1) + CTMP1
                  CALL MPADD (MP_VNOR(1,1), CTMP1, MPM1)
                  CALL MPEQ (MPM1, MP_VNOR(1,1))
!   CMP<
               END IF
!   C      dF/dy
               IF ( J .GT. 0 ) THEN
...
!   CMP>   $                       CTMP2 = CTMP2 * X**I
                  IF (I .GT. 0 )  THEN
                     CALL MPNPWR (X, I, MPM1)
                     CALL MPMUL (CTMP2, MPM1, MPM2)
                     CALL MPEQ (MPM2, CTMP2)
                  ENDIF
!   CMP<
!   CMP>                          IF (J .GT. 1 )
!   CMP>   $                       CTMP2 = J*CTMP2 * Y**(J-1)
                  IF (J .GT. 1 ) THEN
                     MPD1 = J
                     CALL MPMULD (CTMP2, MPD1, 0, MPM1)
                     MPI1 = J - 1
                     CALL MPNPWR (Y, MPI1, MPM2)
                     CALL MPMUL (MPM1, MPM2, MPM3)
                     CALL MPEQ (MPM3, CTMP2)
                  ENDIF
!   CMP<
!   CMP>                          IF (K .GT. 0  )
!   CMP>   $                       CTMP2 = CTMP2 * Z**K
                  ...
                  ENDIF
!   CMP<
!   CMP>                          MP_VNOR(2) = MP_VNOR(2) + CTMP2
                  CALL MPADD (MP_VNOR(1,2), CTMP2, MPM1)
                  CALL MPEQ (MPM1, MP_VNOR(1,2))
!   CMP<
               END IF
!   C      dF/dz
               IF ( K .GT. 0 ) THEN
...
!   CMP>   $                       CTMP3 = CTMP3 * X**I
                  IF (I .GT. 0 ) THEN
                     CALL MPNPWR (X, I, MPM1)
                     CALL MPMUL (CTMP3, MPM1, MPM2)
                     CALL MPEQ (MPM2, CTMP3)
                  ENDIF
!   CMP<
!   CMP>                          IF (J .GT. 0 )
!   CMP>   $                       CTMP3 = CTMP3 * Y**J
...
!   CMP<
!   CMP>                          IF (K .GT. 1 )
!   CMP>   $                       CTMP3 = K*CTMP3 * Z**(K-1)
                  IF (K .GT. 1 )  THEN
                     MPD1 = K
                     CALL MPMULD (CTMP3, MPD1, 0, MPM1)
                     MPI1 = K - 1
                     CALL MPNPWR (Z, MPI1, MPM2)
                     CALL MPMUL (MPM1, MPM2, MPM3)
                     CALL MPEQ (MPM3, CTMP3)
                  ENDIF
!   CMP<
!   CMP>                          MP_VNOR(3) = MP_VNOR(3) + CTMP3
                  CALL MPADD (MP_VNOR(1,3), CTMP3, MPM1)
                  CALL MPEQ (MPM1, MP_VNOR(1,3))
!   CMP<
               END IF
1           CONTINUE
         END DO
      END DO
!   CMP>     VNOR(1) = MP_VNOR(1)
      CALL MPMDC (MP_VNOR(1,1), MPD1, MPI1)
      VNOR(1) = MPD1 * 2.D0 ** MPI1
!   CMP<
!   CMP>     VNOR(2) = MP_VNOR(2)
      CALL MPMDC (MP_VNOR(1,2), MPD1, MPI1)
      VNOR(2) = MPD1 * 2.D0 ** MPI1
!   CMP<
!   CMP>     VNOR(3) = MP_VNOR(3)
      CALL MPMDC (MP_VNOR(1,3), MPD1, MPI1)
      VNOR(3) = MPD1 * 2.D0 ** MPI1
!   CMP<
      return
   END SUBROUTINE MP_POLY_GRAD

!   c-----------------------------------------------------------------------
!   c NEW Version ZRPOLY
   SUBROUTINE MP_ZRPOLY (A,NDEG,Z,IER)
...
!   c                                  SPECIFICATIONS FOR LOCAL VARIABLES
      INTEGER    N, NN, J, JJ, I, NM1, ICNT, N2, L, NZ, NPI, i20
      PARAMETER(i20=20)
      REAL*8 RINFP, REPSP, RADIX, RLO, XX, YY, SINR, COSR, RMAX, RMIN, &
         SC, BND, XXX
      REAL PT(21,101), ZZ(21,MAX_NDEG*2)
      REAL TEMP(21,101), P(21,101), QP(21,101), RK(21,101), QK(21,101), &
         SVK(21,101)
      REAL*8 BB, FACTOR, REPSR1, ZERO, ONE, FN
      LOGICAL               ZEROK
      REAL X(21), XM(21), FF(21), DX(21),DF(21), AA(21), CC(21), T(21), SR(21)
      REAL SI(21), U(21), V(21), RA(21), RB(21), C(21), D(21), A1(21), A2(21)
      REAL A3(21), A6(21), A7(21), E(21), F(21), G(21), H(21), SZR(21), SZI(21)
      REAL RLZR(21), RLZI(21), ETA(21), ARE(21), RMRE(21), MPM1(21)
      LOGICAL MPL1
      REAL MPM2(21)
      INTEGER MPI1
      DOUBLE PRECISION MPD1
      ...
      COMMON /MPTCON/ MPNWQ, MPNW4, MPL02(22), MPL10(22), MPPIC(22)
!   C
!   CMP<
      DATA               RINFP/1.7D+38/
      DATA               REPSP/2.9388D-70/
      DATA               RADIX/2.0D0/
      DATA               REPSR1/2.775557562D-40/
      DATA               ZERO/0.0D0/,ONE/1.0D0/

      IER = 0
      IF (NDEG .GT. 100 .OR. NDEG .LT. 1) GO TO 165
!   CMP>     IF ( ABS(A(1)) .LT. 1D-20  ) THEN
      CALL MPEQ (A(1,1), MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPDMC (1D-20, 0, MPM2)
      CALL MPCPR (MPM1, MPM2, MPI1)
      MPL1 = MPI1 .LT. 0
      IF (MPL1) THEN
!   CMP<
!   CMP>            REPSR1 = ABS(A(1))/1D10
         CALL MPEQ (A(1,1), MPM1)
         MPM1(1) = ABS (MPM1(1))
         CALL MPDIVD (MPM1, 1D10, 0, MPM2)
         CALL MPMDC (MPM2, MPD1, MPI1)
         REPSR1 = MPD1 * 2.D0 ** MPI1
!   CMP<
         REPSP  = REPSR1/1D10
      END IF
!   CMP>     ETA = REPSR1
      CALL MPDMC (REPSR1, 0, ETA)
!   CMP<
!   CMP>     ARE = ETA
      CALL MPEQ (ETA, ARE)
!   CMP<
...
!   CMP<
!   CMP>     RLO = REPSP/ETA
      CALL MPDMC (REPSP, 0, MPM2)
      CALL MPDIV (MPM2, ETA, MPM1)
      CALL MPMDC (MPM1, MPD1, MPI1)
      RLO = MPD1 * 2.D0 ** MPI1
!   c                                  INITIALIZATION OF CONSTANTS FOR
!   c                                    SHIFT ROTATION
      XX = 0.7071067811865475D0
      YY = -XX
      SINR = 0.9975640502598242D0
      COSR = -0.06975647374412530D0
      N = NDEG
      NN = N+1
!   c                                  ALGORITHM FAILS IF THE LEADING
!   c                                    COEFFICIENT IS ZERO.
!   CMP>     IF (A(1).NE.ZERO) GO TO 5
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (A(1,1), MPM1, MPI1)
      MPL1 = MPI1 .NE. 0
      IF (MPL1) THEN
         GO TO 5
      ENDIF
...
!   CMP>    IF (A(NN).NE.ZERO) GO TO 10
5     CONTINUE
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (A(1,NN), MPM1, MPI1)
      MPL1 = MPI1 .NE. 0
      IF (MPL1) THEN
         GO TO 10
      ENDIF
...
!   c                                    ZEROS
      IF (N.EQ.2) GO TO 25
!   CMP>                 ZZ(NDEG) = -P(2)/P(1)
      CALL MPDIV (P(1,2), P(1,1), MPM1)
      CALL MPEQ (MPM1, MPM2)
      MPM2(1) = - MPM2(1)
      CALL MPEQ (MPM2, ZZ(1,NDEG))
!   CMP<
!   CMP>                 ZZ(NDEG+NDEG) = ZERO
      CALL MPDMC (ZERO, 0, ZZ(1,NDEG+NDEG))
!   CMP<
      GO TO 145
!   CMP>     CALL MP_ZRPQLI (P(1),P(2),P(3),ZZ(NDEG-1),ZZ(NDEG+NDEG-1),
!   CMP>   $         ZZ(NDEG),ZZ(NDEG+NDEG))
25    CONTINUE
      CALL MP_ZRPQLI (P(1,1), P(1,2), P(1,3), ZZ(1,NDEG-1), ZZ(1,NDEG+NDEG-1), &
         ZZ(1,NDEG), ZZ(1,NDEG+NDEG))
!   CMP<
      GO TO 145
...
30    RMAX = 0.
      ...
      DO 35 I=1,NN
!CMP>                    X = ABS(P(I))
         CALL MPEQ (P(1,I), MPM1)
         MPM1(1) = ABS (MPM1(1))
         CALL MPEQ (MPM1, X)
!   CMP<
!   CMP>                    IF (X.GT.RMAX) RMAX = X
         CALL MPDMC (RMAX, 0, MPM1)
         CALL MPCPR (X, MPM1, MPI1)
         MPL1 = MPI1 .GT. 0
         IF (MPL1) THEN
            CALL MPMDC (X, MPD1, MPI1)
            RMAX = MPD1 * 2.D0 ** MPI1
         ENDIF
!   CMP<
!   CMP>                    IF (X.NE.0.D0 .AND. X.LT.RMIN) RMIN = X
         CALL MPDMC (0.D0, 0, MPM1)
         CALL MPCPR (X, MPM1, MPI1)
         MPL1 = MPI1 .NE. 0
         CALL MPDMC (RMIN, 0, MPM1)
         CALL MPCPR (X, MPM1, MPI1)
         MPL2 = MPI1 .LT. 0
         MPL3 = MPL1 .AND. MPL2
         IF (MPL3) THEN
...
45    L = LOG(SC)/LOG(RADIX)+.5D0
      IF (L .EQ. 0) GO TO 55
      FACTOR = RADIX**L
      DO 50 I=1,NN
!   CMP>                  P(I) = FACTOR*P(I)
         CALL MPMULD (P(1,I), FACTOR, 0, MPM1)
         CALL MPEQ (MPM1, P(1,I))
!   CMP<
50    CONTINUE
...
!   CMP>                    PT(I) = ABS(P(I))
         CALL MPEQ (P(1,I), MPM1)
         MPM1(1) = ABS (MPM1(1))
         CALL MPEQ (MPM1, PT(1,I))
!   CMP<
60    CONTINUE
!   CMP>                 PT(NN) = -PT(NN)
      CALL MPEQ (PT(1,NN), MPM1)
      MPM1(1) = - MPM1(1)
      CALL MPEQ (MPM1, PT(1,NN))
!   CMP<
!   c                                  COMPUTE UPPER ESTIMATE OF BOUND
!   CMP>                 X = EXP((LOG(-PT(NN))-LOG(PT(1)))/N)
      CALL MPEQ (PT(1,NN), MPM1)
      MPM1(1) = - MPM1(1)
      CALL MPLOG (MPM1, MPL02, MPM2)
      CALL MPLOG (PT(1,1), MPL02, MPM1)
      CALL MPSUB (MPM2, MPM1, MPM3)
      MPD1 = N
      CALL MPDIVD (MPM3, MPD1, 0, MPM1)
      CALL MPEXP (MPM1, MPL02, MPM2)
      CALL MPEQ (MPM2, X)
!   CMP<
!   CMP>                 IF (PT(N).EQ.0.D0) GO TO 65
      CALL MPDMC (0.D0, 0, MPM1)
      CALL MPCPR (PT(1,N), MPM1, MPI1)
      MPL1 = MPI1 .EQ. 0
      IF (MPL1) THEN
         GO TO 65
      ENDIF
...
!   c                                    BETTER, USE IT.
!   CMP>                 XM = -PT(NN)/PT(N)
      CALL MPDIV (PT(1,NN), PT(1,N), MPM1)
      CALL MPEQ (MPM1, MPM2)
      MPM2(1) = - MPM2(1)
      CALL MPEQ (MPM2, XM)
!   CMP<
!   CMP>                 IF (XM.LT.X) X = XM
      CALL MPCPR (XM, X, MPI1)
      MPL1 = MPI1 .LT. 0
      IF (MPL1) THEN
         CALL MPEQ (XM, X)
      ENDIF
...
!   CMP<
      DO 70 I=2,NN
!   CMP>                  FF = FF*XM+PT(I)
         CALL MPMUL (FF, XM, MPM1)
         CALL MPADD (MPM1, PT(1,I), MPM2)
         CALL MPEQ (MPM2, FF)
!   CMP<
70    CONTINUE
!   CMP>                 IF (FF.LE.0.) GO TO 75
      MPD1 = 0.
      CALL MPDMC (MPD1, 0, MPM1)
      CALL MPCPR (FF, MPM1, MPI1)
      MPL1 = MPI1 .LE. 0
      IF (MPL1) THEN
         GO TO 75
      ENDIF
...
!   CMP>     IF (ABS(DX/X).LE..005D0) GO TO 90
80    CONTINUE
      CALL MPDIV (DX, X, MPM1)
      CALL MPEQ (MPM1, MPM2)
      MPM2(1) = ABS (MPM2(1))
      CALL MPDMC (.005D0, 0, MPM1)
      CALL MPCPR (MPM2, MPM1, MPI1)
      MPL1 = MPI1 .LE. 0
      IF (MPL1) THEN
         GO TO 90
      ENDIF
...
!   CMP<
      DO 85 I=2,N
!   CMP>                    FF = FF*X+PT(I)
         CALL MPMUL (FF, X, MPM1)
         CALL MPADD (MPM1, PT(1,I), MPM2)
         CALL MPEQ (MPM2, FF)
!   CMP<
!   CMP>                    DF = DF*X+FF
         CALL MPMUL (DF, X, MPM1)
         CALL MPADD (MPM1, FF, MPM2)
         CALL MPEQ (MPM2, DF)
!   CMP<
85    CONTINUE
!   CMP>                 FF = FF*X+PT(NN)
      CALL MPMUL (FF, X, MPM1)
      CALL MPADD (MPM1, PT(1,NN), MPM2)
      CALL MPEQ (MPM2, FF)
!   CMP<
!   CMP>                 DX = FF/DF
      CALL MPDIV (FF, DF, MPM1)
      CALL MPEQ (MPM1, DX)
!   CMP<
!   CMP>                 X = X-DX
      CALL MPSUB (X, DX, MPM1)
      CALL MPEQ (MPM1, X)
!   CMP<
      GO TO 80
!   CMP>     BND = X
...
!   c                                    NO SHIFT
      ...
      FN = ONE/N
      DO 95 I=2,N
!   CMP>                  RK(I) = (NN-I)*P(I)*FN
         MPI1 = NN - I
         MPD1 = MPI1
         CALL MPMULD (P(1,I), MPD1, 0, MPM1)
         CALL MPMULD (MPM1, FN, 0, MPM2)
         CALL MPEQ (MPM2, RK(1,I))
!   CMP<
95    CONTINUE
!   CMP>                 RK(1) = P(1)
      CALL MPEQ (P(1,1), RK(1,1))
!   CMP<
!   CMP>                 AA = P(NN)
      CALL MPEQ (P(1,NN), AA)
!   CMP<
!   CMP>                 BB = P(N)
      CALL MPMDC (P(1,N), MPD1, MPI1)
      BB = MPD1 * 2.D0 ** MPI1
!   CMP<
!   CMP>                 ZEROK = RK(N).EQ.ZERO
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (RK(1,N), MPM1, MPI1)
      MPL1 = MPI1 .EQ. 0
      ZEROK = MPL1
!   CMP<
      DO 115 JJ=1,5
...
!   CMP<
         DO 100 I=1,NM1
            J = NN-I
!   CMP>                       RK(J) = T*RK(J-1)+P(J)
            CALL MPMUL (T, RK(1,J-1), MPM1)
            CALL MPADD (MPM1, P(1,J), MPM2)
            CALL MPEQ (MPM2, RK(1,J))
!   CMP<
100      CONTINUE
!   CMP>                    RK(1) = P(1)
         CALL MPEQ (P(1,1), RK(1,1))
!   CMP<
!   CMP>                    ZEROK = ABS(RK(N)).LE.ABS(BB)*ETA*10.D0
         CALL MPEQ (RK(1,N), MPM1)
         MPM1(1) = ABS (MPM1(1))
         MPD1 = ABS (BB)
         CALL MPMULD (ETA, MPD1, 0, MPM2)
         CALL MPMULD (MPM2, 10.D0, 0, MPM3)
...
105      DO 110 I=1,NM1
            J = NN-I
!   CMP>                       RK(J) = RK(J-1)
            CALL MPEQ (RK(1,J-1), RK(1,J))
!   CMP<
110      CONTINUE
!   CMP>                   RK(1) = ZERO
         CALL MPDMC (ZERO, 0, RK(1,1))
!   CMP<
!   CMP>                   ZEROK = RK(N).EQ.ZERO
         CALL MPDMC (ZERO, 0, MPM1)
         CALL MPCPR (RK(1,N), MPM1, MPI1)
...
!   CMP>                    SR = BND*XX
         MPD1 = BND * XX
         CALL MPDMC (MPD1, 0, SR)
!   CMP<
!   CMP>                    SI = BND*YY
         MPD1 = BND * YY
         CALL MPDMC (MPD1, 0, SI)
!   CMP<
!   CMP>                    U = -SR-SR
         CALL MPEQ (SR, MPM1)
         MPM1(1) = - MPM1(1)
         CALL MPSUB (MPM1, SR, MPM2)
         CALL MPEQ (MPM2, U)
!   CMP<
!   CMP>                    V = BND*BND
         MPD1 = BND * BND
...
!   c                                  DEFLATE THE POLYNOMIAL, STORE THE
         ...
         JJ = J+NDEG
!   CMP>                    ZZ(J) = SZR
         CALL MPEQ (SZR, ZZ(1,J))
!   CMP<
!   CMP>                    ZZ(JJ) = SZI
         CALL MPEQ (SZI, ZZ(1,JJ))
!   CMP<
         NN = NN-NZ
         N = NN-1
         DO 125 I=1,NN
!   CMP>                     P(I) = QP(I)
            CALL MPEQ (QP(1,I), P(1,I))
!   CMP<
125      CONTINUE
         IF (NZ.EQ.1) GO TO 20
!   CMP>                    ZZ(J+1) = RLZR
         CALL MPEQ (RLZR, ZZ(1,J+1))
!   CMP<
!   CMP>                    ZZ(JJ+1) = RLZI
         CALL MPEQ (RLZI, ZZ(1,JJ+1))
!   CMP<
         GO TO 20
...
150   CONTINUE
      ...
      DO 155 I=1,NDEG
         ! CMP>                    Z(N2-1) = ZZ(J)
         CALL MPMDC (ZZ(1,J), MPD1, MPI1)
         Z(N2-1) = MPD1 * 2.D0 ** MPI1
         ! CMP<
         ! CMP>                    Z(N2) = P(J)
         CALL MPMDC (P(1,J), MPD1, MPI1)
         Z(N2) = MPD1 * 2.D0 ** MPI1
         ! CMP<
         N2 = N2-2
         J = J-1
155   CONTINUE
      ...
      DO 160 I=1,N
         ! CMP>                    ZZ(N2) = RINFP
         CALL MPDMC (RINFP, 0, ZZ(1,N2))
         ! CMP<
         ! CMP>                    ZZ(N2+1) = RINFP
         CALL MPDMC (RINFP, 0, ZZ(1,N2+1))
         ! CMP<
         N2 = N2+2
160   CONTINUE
      GO TO 9000
...
9000  CONTINUE
      ! CMP>            XX = A(1)
      CALL MPMDC (A(1,1), MPD1, MPI1)
      XX = MPD1 * 2.D0 ** MPI1
      ! CMP<
      print *, &
         "Error from math routine zrpoly. Called with NDEG,A: ", &
         NDEG,XX
9005  RETURN
   END SUBROUTINE MP_ZRPOLY

! New MP version ZRPQLB
   SUBROUTINE MP_ZRPQLB (L2,NZ)
      ...
      ! CMP<
      ! c                                  SPECIFICATIONS FOR LOCAL VARIABLES
      ! CMP>                 INTEGER      N,NN,J,ITYPE,I,IFLAG
      INTEGER       N, NN, J, ITYPE, I, IFLAG
      REAL*8 BETAS, BETAV, TSS, TVV
      REAL P(21,101), QP(21,101), RK(21,101), QK(21,101), SVK(21,101)
      REAL*8 SVU, SVV, ZERO
      LOGICAL               VPASS, SPASS, VTRY, STRY
      REAL SS(21),TV(21),VV(21), OVV(21), TS(21), S(21), UI(21), VI(21), OSS(21)
      REAL OTS(21), OTV(21), SR(21)
      REAL SI(21)
      REAL U(21)
      REAL V(21)
      REAL RA(21)
      ...
      INTEGER MPI1
      REAL MPM2(21)
      COMMON /ZRPQLJ/ P,QP,RK,QK,SVK,SR,SI,U,V,RA,RB,C,D, &
         A1,A2,A3,A6,A7,E,F,G,H,SZR,SZI,RLZR,RLZI,ETA,ARE, &
         RMRE,N,NN
      ...
      ! c                                    DIVISION
      ! CMP>                 CALL MP_ZRPQLH (NN,U,V,P,QP,RA,RB)
      CALL MP_ZRPQLH (NN, U, V, P, QP, RA, RB)
      ! CMP<
      CALL MP_ZRPQLE (ITYPE)
      DO 40 J=1,L2
         ! c                                  CALCULATE NEXT K POLYNOMIAL AND
         ! c                                    ESTIMATE V
         CALL MP_ZRPQLF (ITYPE)
         CALL MP_ZRPQLE (ITYPE)
         ! CMP>            CALL MP_ZRPQLG (ITYPE,UI,VI)
         CALL MP_ZRPQLG (ITYPE, UI, VI)
         ! CMP<
         ! CMP>            VV = VI
         CALL MPEQ (VI, VV)
         ! CMP<
         ...
         ! CMP>            SS = 0.
         MPD1 = 0.
         CALL MPDMC (MPD1, 0, SS)
         ! CMP<
         ! CMP>            IF (RK(N).NE.ZERO) SS = -P(NN)/RK(N)
         CALL MPDMC (ZERO, 0, MPM1)
         CALL MPCPR (RK(1,N), MPM1, MPI1)
         MPL1 = MPI1 .NE. 0
         IF (MPL1) THEN
            CALL MPDIV (P(1,NN), RK(1,N), MPM1)
            CALL MPEQ (MPM1, MPM2)
            MPM2(1) = - MPM2(1)
            CALL MPEQ (MPM2, SS)
         ENDIF
         ...
         ! CMP>            TV = 1.
         MPD1 = 1.
         CALL MPDMC (MPD1, 0, TV)
         ! CMP<
         ! CMP>            TS = 1.
         MPD1 = 1.
         CALL MPDMC (MPD1, 0, TS)
         ! CMP<
         IF (J.EQ.1 .OR. ITYPE.EQ.3) GO TO 35
         ! c             COMPUTE RELATIVE MEASURES OF
         ! c             CONVERGENCE OF S AND V SEQUENCES
         ! CMP>            IF (VV.NE.0.) TV = ABS((VV-OVV)/VV)
         MPD1 = 0.
         CALL MPDMC (MPD1, 0, MPM1)
         CALL MPCPR (VV, MPM1, MPI1)
         MPL1 = MPI1 .NE. 0
         IF (MPL1) THEN
            CALL MPSUB (VV, OVV, MPM1)
            CALL MPDIV (MPM1, VV, MPM2)
            CALL MPEQ (MPM2, MPM1)
            MPM1(1) = ABS (MPM1(1))
            CALL MPEQ (MPM1, TV)
         ENDIF
         ! CMP<
         ! CMP>            IF (SS.NE.0.) TS = ABS((SS-OSS)/SS)
         MPD1 = 0.
         CALL MPDMC (MPD1, 0, MPM1)
         CALL MPCPR (SS, MPM1, MPI1)
         MPL1 = MPI1 .NE. 0
         IF (MPL1) THEN
            CALL MPSUB (SS, OSS, MPM1)
            CALL MPDIV (MPM1, SS, MPM2)
            CALL MPEQ (MPM2, MPM1)
            MPM1(1) = ABS (MPM1(1))
            CALL MPEQ (MPM1, TS)
         ENDIF
         ...
         TVV = 1.
         ! CMP>            IF (TV.LT.OTV) TVV = TV*OTV
         CALL MPCPR (TV, OTV, MPI1)
         MPL1 = MPI1 .LT. 0
         IF (MPL1) THEN
            CALL MPMUL (TV, OTV, MPM1)
            CALL MPMDC (MPM1, MPD1, MPI1)
            TVV = MPD1 * 2.D0 ** MPI1
         ENDIF
         ! CMP<
         TSS = 1.
         ! CMP>            IF (TS.LT.OTS) TSS = TS*OTS
         CALL MPCPR (TS, OTS, MPI1)
         MPL1 = MPI1 .LT. 0
         IF (MPL1) THEN
            CALL MPMUL (TS, OTS, MPM1)
            CALL MPMDC (MPM1, MPD1, MPI1)
            TSS = MPD1 * 2.D0 ** MPI1
         ENDIF
         ! CMP<
         ...
         ! c                                    BEFORE ITERATING
         ! CMP>            SVU = U
         CALL MPMDC (U, MPD1, MPI1)
         SVU = MPD1 * 2.D0 ** MPI1
         ! CMP<
         ! CMP>            SVV = V
         CALL MPMDC (V, MPD1, MPI1)
         SVV = MPD1 * 2.D0 ** MPI1
         ! CMP<
         DO 5 I=1,N
            ! CMP>            SVK(I) = RK(I)
            CALL MPEQ (RK(1,I), SVK(1,I))
            ! CMP<
5        CONTINUE
         ! CMP>                    S = SS
         CALL MPEQ (SS, S)
         ...
         VTRY = .FALSE.
         STRY = .FALSE.
         IF (SPASS.AND.((.NOT.VPASS).OR.TSS.LT.TVV)) &
            GO TO 20
         ! CMP>     CALL MP_ZRPQLC (UI,VI,NZ)
10       CONTINUE
         CALL MP_ZRPQLC (UI, VI, NZ)
         ! CMP<
         IF (NZ.GT.0) RETURN
         ...
         DO 15 I=1,N
            ! CMP>                     RK(I) = SVK(I)
            CALL MPEQ (SVK(1,I), RK(1,I))
            ! CMP<
15       CONTINUE
         ! CMP>     CALL MP_ZRPQLD (S,NZ,IFLAG)
20       CONTINUE
         CALL MP_ZRPQLD (S, NZ, IFLAG)
         ! CMP<
         IF (NZ.GT.0) RETURN
...
40    CONTINUE
      RETURN
   END SUBROUTINE MP_ZRPQLB


   ! c-----------------------------------------------------------------------
   ! new mp version MP_ZRPQLC
   SUBROUTINE MP_ZRPQLC (UU,VV,NZ)
      ! c                                  SPECIFICATIONS FOR ARGUMENTS
      ! CMP>                 INTEGER       NZ
      REAL UU(21), VV(21)
      INTEGER        NZ
...
5     CONTINUE
      CALL MPDMC (ONE, 0, UI)
      ! CMP<
      ! CMP>    CALL MP_ZRPQLI (UI,U,V,SZR,SZI,RLZR,RLZI)
      CALL MP_ZRPQLI (UI, U, V, SZR, SZI, RLZR, RLZI)
      ! CMP<
      ! c                                  RETURN IF ROOTS OF THE QUADRATIC ARE
      ! c                                    REAL AND NOT CLOSE TO MULTIPLE OR
      ! c                                    NEARLY EQUAL AND OF OPPOSITE SIGN
      ! CMP>                 IF ( ABS(ABS(SZR)-ABS(RLZR)).GT.PT01*ABS(RLZR))
      ! CMP>   $             RETURN
      CALL MPEQ (SZR, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (RLZR, MPM2)
      MPM2(1) = ABS (MPM2(1))
      CALL MPSUB (MPM1, MPM2, MPM3)
      CALL MPEQ (MPM3, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (RLZR, MPM2)
      MPM2(1) = ABS (MPM2(1))
      CALL MPMULD (MPM2, PT01, 0, MPM3)
      CALL MPCPR (MPM1, MPM3, MPI1)
      MPL1 = MPI1 .GT. 0
      IF (MPL1) THEN
         RETURN
      ...
      CALL MP_ZRPQLH (NN, U, V, P, QP, RA, RB)
      ! CMP<
      ! CMP>                 RMP = ABS(RA-SZR*RB)+ABS(SZI*RB)
      CALL MPMUL (SZR, RB, MPM1)
      CALL MPSUB (RA, MPM1, MPM2)
      CALL MPEQ (MPM2, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPMUL (SZI, RB, MPM2)
      CALL MPEQ (MPM2, MPM3)
      MPM3(1) = ABS (MPM3(1))
      CALL MPADD (MPM1, MPM3, MPM2)
      CALL MPEQ (MPM2, RMP)
      ! CMP<
      ...
      CALL MPEQ (V, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPSQRT (MPM1, MPM2)
      CALL MPEQ (MPM2, ZM)
      ! CMP<
      ! CMP>                 EE = 2.*ABS(QP(1))
      CALL MPEQ (QP(1,1), MPM1)
      MPM1(1) = ABS (MPM1(1))
      MPD1 = 2.
      ...
      ! CMP<
      DO 10 I=2,N
         ! CMP>                  EE = EE*ZM+ABS(QP(I))
         CALL MPMUL (EE, ZM, MPM1)
         CALL MPEQ (QP(1,I), MPM2)
         MPM2(1) = ABS (MPM2(1))
         CALL MPADD (MPM1, MPM2, MPM3)
         CALL MPEQ (MPM3, EE)
         ! CMP<
10    CONTINUE
      ! CMP>                 EE = EE*ZM+ABS(RA+T)
      CALL MPMUL (EE, ZM, MPM1)
      CALL MPADD (RA, T, MPM2)
      CALL MPEQ (MPM2, MPM3)
      MPM3(1) = ABS (MPM3(1))
      CALL MPADD (MPM1, MPM3, MPM2)
      CALL MPEQ (MPM2, EE)
      ! CMP<
      ! CMP>                 EE = (5.*RMRE+4.*ARE)*EE-(5.*RMRE+2.*ARE)*
      ! CMP>   $              (ABS(RA+T)+
      ! CMP>   $                       ABS(RB)*ZM)+2.*ARE*ABS(T)
      MPD1 = 5.
      CALL MPMULD (RMRE, MPD1, 0, MPM1)
      MPD1 = 4.
      CALL MPMULD (ARE, MPD1, 0, MPM2)
      CALL MPADD (MPM1, MPM2, MPM3)
      CALL MPMUL (MPM3, EE, MPM1)
      MPD1 = 5.
      CALL MPMULD (RMRE, MPD1, 0, MPM2)
      MPD1 = 2.
      CALL MPMULD (ARE, MPD1, 0, MPM3)
      CALL MPADD (MPM2, MPM3, MPM4)
      CALL MPADD (RA, T, MPM2)
      CALL MPEQ (MPM2, MPM3)
      MPM3(1) = ABS (MPM3(1))
      CALL MPEQ (RB, MPM2)
      MPM2(1) = ABS (MPM2(1))
      CALL MPMUL (MPM2, ZM, MPM5)
      CALL MPADD (MPM3, MPM5, MPM2)
      CALL MPMUL (MPM4, MPM2, MPM3)
      CALL MPSUB (MPM1, MPM3, MPM2)
      MPD1 = 2.
      CALL MPMULD (ARE, MPD1, 0, MPM1)
      CALL MPEQ (T, MPM3)
      MPM3(1) = ABS (MPM3(1))
      CALL MPMUL (MPM1, MPM3, MPM4)
      CALL MPADD (MPM2, MPM4, MPM1)
      CALL MPEQ (MPM1, EE)
      ! CMP<
      ! c                                  ITERATION HAS CONVERGED SUFFICIENTLY
      ! c                                    IF THE POLYNOMIAL VALUE IS LESS
      ! c                                    THAN 20 TIMES THIS BOUND
      ! CMP>                 IF (RMP.GT.20.*EE) GO TO 15
      MPD1 = 20.
      CALL MPMULD (EE, MPD1, 0, MPM1)
      CALL MPCPR (RMP, MPM1, MPI1)
      MPL1 = MPI1 .GT. 0
      IF (MPL1) THEN
         GO TO 15
      ENDIF
...
15    J = J+1
      ...
      IF (J.GT.20) RETURN
      IF (J.LT.2) GO TO 25
      ! CMP>            IF (RELSTP.GT..01 .OR. RMP.LT.OMP .OR. TRIED) GO TO 25
      MPL1 = RELSTP .GT. .01
      CALL MPCPR (RMP, OMP, MPI1)
      MPL2 = MPI1 .LT. 0
      MPL3 = MPL1 .OR. MPL2
      MPL1 = MPL3 .OR. TRIED
      IF (MPL1) THEN
      ...
      ! c                                    TO THE CLUSTER
      ! CMP>                 IF (RELSTP.LT.ETA) RELSTP = ETA
      CALL MPDMC (RELSTP, 0, MPM1)
      CALL MPCPR (MPM1, ETA, MPI1)
      MPL1 = MPI1 .LT. 0
      IF (MPL1) THEN
         CALL MPMDC (ETA, MPD1, MPI1)
         RELSTP = MPD1 * 2.D0 ** MPI1
      ENDIF
      ! CMP<
      RELSTP = SQRT(RELSTP)
      ! CMP>                 U = U-U*RELSTP
      CALL MPMULD (U, RELSTP, 0, MPM1)
      CALL MPSUB (U, MPM1, MPM2)
      CALL MPEQ (MPM2, U)
      ! CMP<
      ! CMP>                 V = V+V*RELSTP
      CALL MPMULD (V, RELSTP, 0, MPM1)
      CALL MPADD (V, MPM1, MPM2)
      CALL MPEQ (MPM2, V)
      ! CMP<
      ! CMP>                 CALL MP_ZRPQLH (NN,U,V,P,QP,RA,RB)
      CALL MP_ZRPQLH (NN, U, V, P, QP, RA, RB)
...
25    CONTINUE
      ...
      ! c                                    CONVERGING
      ! CMP>                 IF (VI.EQ.ZERO) RETURN
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (VI, MPM1, MPI1)
      MPL1 = MPI1 .EQ. 0
      IF (MPL1) THEN
         RETURN
      ENDIF
      ! CMP<
      ! CMP>                 RELSTP = ABS((VI-V)/VI)
      CALL MPSUB (VI, V, MPM1)
      CALL MPDIV (MPM1, VI, MPM2)
      CALL MPEQ (MPM2, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPMDC (MPM1, MPD1, MPI1)
      RELSTP = MPD1 * 2.D0 ** MPI1
   ...
   END SUBROUTINE MP_ZRPQLC

!New MP version  ZRPQLD
   SUBROUTINE MP_ZRPQLD (SSS,NZ,IFLAG)
      REAL SSS(21)
      INTEGER        NZ, IFLAG
!   c             SPECIFICATIONS FOR LOCAL VARIABLES
      INTEGER       N, NN, J, I
...
5     CONTINUE
      ...
      DO 10 I=2,NN
         ! CMP>                          PV = PV*S+P(I)
         CALL MPMUL (PV, S, MPM1)
         CALL MPADD (MPM1, P(1,I), MPM2)
         CALL MPEQ (MPM2, PV)
         ! CMP<
         ! CMP>                    QP(I) = PV
         CALL MPEQ (PV, QP(1,I))
...
10    CONTINUE
      ...
      CALL MPEQ (MPM1, RMS)
      ! CMP<
      ! CMP>                 EE = (RMRE/(ARE+RMRE))*ABS(QP(1))
      CALL MPADD (ARE, RMRE, MPM1)
      CALL MPDIV (RMRE, MPM1, MPM2)
      CALL MPEQ (QP(1,1), MPM1)
      MPM1(1) = ABS (MPM1(1))
      ...
      ! CMP<
      DO 15 I=2,NN
         ! CMP>                  EE = EE*RMS+ABS(QP(I))
         CALL MPMUL (EE, RMS, MPM1)
         CALL MPEQ (QP(1,I), MPM2)
         MPM2(1) = ABS (MPM2(1))
         CALL MPADD (MPM1, MPM2, MPM3)
         CALL MPEQ (MPM3, EE)
...
15    CONTINUE
      ...
      ! c                                    IF THE POLYNOMIAL VALUE IS LESS
      ! c                                    THAN 20 TIMES THIS BOUND
      ! CMP>                 IF (RMP.GT.20.*((ARE+RMRE)*EE-RMRE*RMP)) GO TO 20
      CALL MPADD (ARE, RMRE, MPM1)
      CALL MPMUL (MPM1, EE, MPM2)
      CALL MPMUL (RMRE, RMP, MPM1)
      CALL MPSUB (MPM2, MPM1, MPM3)
      MPD1 = 20.
      CALL MPMULD (MPM3, MPD1, 0, MPM1)
      CALL MPCPR (RMP, MPM1, MPI1)
      MPL1 = MPI1 .GT. 0
      IF (MPL1) THEN
         GO TO 20
...
20    J = J+1
      ...
      IF (J.GT.10) RETURN
      IF (J.LT.2) GO TO 25
      ! CMP>            IF (ABS(T).GT.PT001*ABS(S-T) .OR. RMP.LE.OMP) GO TO 25
      CALL MPEQ (T, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPSUB (S, T, MPM2)
      CALL MPEQ (MPM2, MPM3)
      MPM3(1) = ABS (MPM3(1))
      CALL MPMULD (MPM3, PT001, 0, MPM2)
      CALL MPCPR (MPM1, MPM2, MPI1)
      MPL1 = MPI1 .GT. 0
      CALL MPCPR (RMP, OMP, MPI1)
      MPL2 = MPI1 .LE. 0
...
25    CONTINUE
      ...
      ! c                                    THE NEW ITERATE
      ! CMP>                 RKV = RK(1)
      CALL MPEQ (RK(1,1), RKV)
      ! CMP<
      ! CMP>                 QK(1) = RKV
      CALL MPEQ (RKV, QK(1,1))
      ! CMP<
      DO 30 I=2,N
         ! CMP>                    RKV = RKV*S+RK(I)
         CALL MPMUL (RKV, S, MPM1)
         CALL MPADD (MPM1, RK(1,I), MPM2)
         CALL MPEQ (MPM2, RKV)
         ! CMP<
         ! CMP>                    QK(I) = RKV
         CALL MPEQ (RKV, QK(1,I))
         ! CMP<
30    CONTINUE
      ! CMP>                 IF (ABS(RKV).LE.ABS(RK(N))*10.*ETA) GO TO 40
      CALL MPEQ (RKV, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (RK(1,N), MPM2)
      MPM2(1) = ABS (MPM2(1))
      MPD1 = 10.
      CALL MPMULD (MPM2, MPD1, 0, MPM3)
      CALL MPMUL (MPM3, ETA, MPM2)
      CALL MPCPR (MPM1, MPM2, MPI1)
      MPL1 = MPI1 .LE. 0
      IF (MPL1) THEN
         GO TO 40
      ...
      ! c                                    IS NONZERO
      ! CMP>                 T = -PV/RKV
      CALL MPDIV (PV, RKV, MPM1)
      CALL MPEQ (MPM1, MPM2)
      MPM2(1) = - MPM2(1)
      CALL MPEQ (MPM2, T)
      ! CMP<
      ! CMP>                 RK(1) = QP(1)
      CALL MPEQ (QP(1,1), RK(1,1))
      ! CMP<
      DO 35 I=2,N
         ! CMP>                  RK(I) = T*QK(I-1)+QP(I)
         CALL MPMUL (T, QK(1,I-1), MPM1)
         CALL MPADD (MPM1, QP(1,I), MPM2)
         CALL MPEQ (MPM2, RK(1,I))
         ! CMP<
35    CONTINUE
      GO TO 50
...
55    CONTINUE
      ...
      CALL MPDMC (ZERO, 0, T)
      ! CMP<
      ! CMP>                 IF (ABS(RKV).GT.ABS(RK(N))*10.*ETA) T = -PV/RKV
      CALL MPEQ (RKV, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (RK(1,N), MPM2)
      MPM2(1) = ABS (MPM2(1))
      ...
      ! CMP<
      GO TO 5
   END SUBROUTINE MP_ZRPQLD

   ! c-----------------------------------------------------------------------
   ! new MP version ZRPQLE

   SUBROUTINE MP_ZRPQLE (ITYPE)
      INTEGER       ITYPE
      ! c                                  SPECIFICATIONS FOR LOCAL VARIABLES
      INTEGER       N, NN
      REAL P(21,101), QP(21,101), RK(21,101), QK(21,101), SVK(21,101)
      ...
      ! CMP<
      ! CMP>                 CALL MP_ZRPQLH (N,U,V,RK,QK,C,D)
      CALL MP_ZRPQLH (N, U, V, RK, QK, C, D)
      ! CMP<
      ! CMP>                 IF (ABS(C).GT.ABS(RK(N))*100.*ETA) GO TO 5
      CALL MPEQ (C, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (RK(1,N), MPM2)
      MPM2(1) = ABS (MPM2(1))
      ...
      ENDIF
      ! CMP<
      ! CMP>                 IF (ABS(D).GT.ABS(RK(N-1))*100.*ETA) GO TO 5
      CALL MPEQ (D, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (RK(1,N-1), MPM2)
      MPM2(1) = ABS (MPM2(1))
      MPD1 = 100.
      CALL MPMULD (MPM2, MPD1, 0, MPM3)
      CALL MPMUL (MPM3, ETA, MPM2)
      ...
      RETURN
      ! CMP>     IF (ABS(D).LT.ABS(C)) GO TO 10
5     CONTINUE
      CALL MPEQ (D, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (C, MPM2)
      ...
      ! CMP<
      ! CMP>                 G = U*RB
      CALL MPMUL (U, RB, MPM1)
      CALL MPEQ (MPM1, G)
      ! CMP<
      ! CMP>                 H = V*RB
      CALL MPMUL (V, RB, MPM1)
      CALL MPEQ (MPM1, H)
      ! CMP<
      ! CMP>                 A3 = (RA+G)*E+H*(RB/D)
      CALL MPADD (RA, G, MPM1)
      CALL MPMUL (MPM1, E, MPM2)
      CALL MPDIV (RB, D, MPM1)
      CALL MPMUL (H, MPM1, MPM3)
      CALL MPADD (MPM2, MPM3, MPM1)
      CALL MPEQ (MPM1, A3)
      ! CMP<
      ! CMP>                 A1 = RB*F-RA
      CALL MPMUL (RB, F, MPM1)
      CALL MPSUB (MPM1, RA, MPM2)
      CALL MPEQ (MPM2, A1)
      ! CMP<
      ! CMP>                 A7 = (F+U)*RA+H
      CALL MPADD (F, U, MPM1)
      CALL MPMUL (MPM1, RA, MPM2)
      CALL MPADD (MPM2, H, MPM1)
      CALL MPEQ (MPM1, A7)
      ! CMP<
      RETURN
10    ITYPE = 1
      ...
      CALL MPEQ (MPM1, H)
      ! CMP<
      ! CMP>                 A3 = RA*E+(H/C+G)*RB
      CALL MPMUL (RA, E, MPM1)
      CALL MPDIV (H, C, MPM2)
      CALL MPADD (MPM2, G, MPM3)
      CALL MPMUL (MPM3, RB, MPM2)
      CALL MPADD (MPM1, MPM2, MPM3)
      CALL MPEQ (MPM3, A3)
      ! CMP<
      ! CMP>                 A1 = RB-RA*(D/C)
      CALL MPDIV (D, C, MPM1)
      CALL MPMUL (RA, MPM1, MPM2)
      CALL MPSUB (RB, MPM2, MPM1)
      CALL MPEQ (MPM1, A1)
      ! CMP<
      ! CMP>                 A7 = RA+G*D+H*F
      CALL MPMUL (G, D, MPM1)
      CALL MPADD (RA, MPM1, MPM2)
      CALL MPMUL (H, F, MPM1)
      CALL MPADD (MPM2, MPM1, MPM3)
      CALL MPEQ (MPM3, A7)
      ! CMP<
      RETURN
   END SUBROUTINE MP_ZRPQLE

!New MP version  ZRPQLF
   SUBROUTINE MP_ZRPQLF (ITYPE)
      INTEGER       ITYPE
      ! c                                  SPECIFICATIONS FOR LOCAL VARIABLES
      ! CMP>                 INTEGER      N,NN,I
      INTEGER       N, NN, I
      ...
      CALL MPEQ (RA, TEMP)
      ! CMP<
      ! CMP>                 IF (ITYPE.EQ.1) TEMP = RB
      IF (ITYPE.EQ.1)  THEN
         CALL MPEQ (RB, TEMP)
      ENDIF
      ! CMP<
      ! CMP>                 IF (ABS(A1).GT.ABS(TEMP)*ETA*10.) GO TO 10
      CALL MPEQ (A1, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (TEMP, MPM2)
      MPM2(1) = ABS (MPM2(1))
      CALL MPMUL (MPM2, ETA, MPM3)
      MPD1 = 10.
      CALL MPMULD (MPM3, MPD1, 0, MPM2)
      CALL MPCPR (MPM1, MPM2, MPI1)
      MPL1 = MPI1 .GT. 0
      IF (MPL1) THEN
         GO TO 10
      ...
      ! CMP<
      ! CMP>                 RK(2) = -A7*QP(1)
      CALL MPMUL (A7, QP(1,1), MPM1)
      CALL MPEQ (MPM1, MPM2)
      MPM2(1) = - MPM2(1)
      CALL MPEQ (MPM2, RK(1,2))
      ! CMP<
      DO 5 I=3,N
         ! CMP>                 RK(I) = A3*QK(I-2)-A7*QP(I-1)
         CALL MPMUL (A3, QK(1,I-2), MPM1)
         CALL MPMUL (A7, QP(1,I-1), MPM2)
         CALL MPSUB (MPM1, MPM2, MPM3)
         CALL MPEQ (MPM3, RK(1,I))
         ! CMP<
5     CONTINUE
      RETURN
      ...
      ! CMP>     A7 = A7/A1
10    CONTINUE
      CALL MPDIV (A7, A1, MPM1)
      CALL MPEQ (MPM1, A7)
      ! CMP<
      ! CMP>                A3 = A3/A1
      CALL MPDIV (A3, A1, MPM1)
      CALL MPEQ (MPM1, A3)
      ! CMP<
      ! CMP>                 RK(1) = QP(1)
      CALL MPEQ (QP(1,1), RK(1,1))
      ! CMP<
      ! CMP>                 RK(2) = QP(2)-A7*QP(1)
      CALL MPMUL (A7, QP(1,1), MPM1)
      CALL MPSUB (QP(1,2), MPM1, MPM2)
      CALL MPEQ (MPM2, RK(1,2))
      ! CMP<
      DO 15 I=3,N
         ! CMP>                  RK(I) = A3*QK(I-2)-A7*QP(I-1)+QP(I)
         CALL MPMUL (A3, QK(1,I-2), MPM1)
         CALL MPMUL (A7, QP(1,I-1), MPM2)
         CALL MPSUB (MPM1, MPM2, MPM3)
         CALL MPADD (MPM3, QP(1,I), MPM1)
         CALL MPEQ (MPM1, RK(1,I))
         ! CMP<
15    CONTINUE
      RETURN
...
20    CONTINUE
      ...
      ! CMP<
      ! CMP>                 RK(2) = ZERO
      CALL MPDMC (ZERO, 0, RK(1,2))
      ! CMP<
      DO 25 I=3,N
         ! CMP>                  RK(I) = QK(I-2)
         CALL MPEQ (QK(1,I-2), RK(1,I))
         ! CMP<
25    CONTINUE
      RETURN
   END SUBROUTINE MP_ZRPQLF

   ! c-----------------------------------------------------------------------
   ! c new MP version ZRPQLG
   SUBROUTINE MP_ZRPQLG (ITYPE,UU,VV)
      INTEGER       ITYPE
      REAL UU(21), VV(21)
      ! c                                  SPECIFICATIONS FOR LOCAL VARIABLES
      INTEGER       N, NN
      ...
      COMMON /ZRPQLJ/P,QP,RK,QK,SVK,SR,SI,U,V,RA,RB,C,D,A1,A2, &
         A3,A6,A7,E,F,G,H,SZR,SZI,RLZR,RLZI,ETA,ARE,RMRE,N,NN
      ...
      ! CMP>                 A5 = C+(U+V*F)*D
      CALL MPMUL (V, F, MPM1)
      CALL MPADD (U, MPM1, MPM2)
      CALL MPMUL (MPM2, D, MPM1)
      CALL MPADD (C, MPM1, MPM2)
      CALL MPEQ (MPM2, A5)
      ! CMP<
      GO TO 10
      ! CMP>     A4 = (RA+G)*F+H
5     CONTINUE
      CALL MPADD (RA, G, MPM1)
      CALL MPMUL (MPM1, F, MPM2)
      CALL MPADD (MPM2, H, MPM1)
      CALL MPEQ (MPM1, A4)
      ! CMP<
      ! CMP>                 A5 = (F+U)*C+V*D
      CALL MPADD (F, U, MPM1)
      CALL MPMUL (MPM1, C, MPM2)
      CALL MPMUL (V, D, MPM1)
      CALL MPADD (MPM2, MPM1, MPM3)
      CALL MPEQ (MPM3, A5)
      ! CMP<
      ...
      ! CMP>     B1 = -RK(N)/P(NN)
10    CONTINUE
      CALL MPDIV (RK(1,N), P(1,NN), MPM1)
      CALL MPEQ (MPM1, MPM2)
      MPM2(1) = - MPM2(1)
      CALL MPEQ (MPM2, B1)
      ! CMP<
      ! CMP>                 B2 = -(RK(N-1)+B1*P(N))/P(NN)
      CALL MPMUL (B1, P(1,N), MPM1)
      CALL MPADD (RK(1,N-1), MPM1, MPM2)
      CALL MPDIV (MPM2, P(1,NN), MPM1)
      CALL MPEQ (MPM1, MPM2)
      MPM2(1) = - MPM2(1)
      CALL MPEQ (MPM2, B2)
      ! CMP<
      ! CMP>                 C1 = V*B2*A1
      CALL MPMUL (V, B2, MPM1)
      CALL MPMUL (MPM1, A1, MPM2)
      CALL MPEQ (MPM2, C1)
      ! CMP<
      ! CMP>                 C2 = B1*A7
      CALL MPMUL (B1, A7, MPM1)
      CALL MPEQ (MPM1, C2)
      ! CMP<
      ! CMP>                 C3 = B1*B1*A3
      CALL MPMUL (B1, B1, MPM1)
      CALL MPMUL (MPM1, A3, MPM2)
      CALL MPEQ (MPM2, C3)
      ! CMP<
      ! CMP>                 C4 = C1-C2-C3
      CALL MPSUB (C1, C2, MPM1)
      CALL MPSUB (MPM1, C3, MPM2)
      CALL MPEQ (MPM2, C4)
      ! CMP<
      ! CMP>                 TEMP = A5+B1*A4-C4
      CALL MPMUL (B1, A4, MPM1)
      CALL MPADD (A5, MPM1, MPM2)
      CALL MPSUB (MPM2, C4, MPM1)
      CALL MPEQ (MPM1, TEMP)
      ! CMP<
      ! CMP>                 IF (TEMP.EQ.ZERO) GO TO 15
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (TEMP, MPM1, MPI1)
      MPL1 = MPI1 .EQ. 0
      IF (MPL1) THEN
         GO TO 15
      ENDIF
      ! CMP<
      ! CMP>                 UU = U-(U*(C3+C2)+V*(B1*A1+B2*A7))/TEMP
      CALL MPADD (C3, C2, MPM1)
      CALL MPMUL (U, MPM1, MPM2)
      CALL MPMUL (B1, A1, MPM1)
      CALL MPMUL (B2, A7, MPM3)
      CALL MPADD (MPM1, MPM3, MPM4)
      CALL MPMUL (V, MPM4, MPM1)
      CALL MPADD (MPM2, MPM1, MPM3)
      CALL MPDIV (MPM3, TEMP, MPM1)
      CALL MPSUB (U, MPM1, MPM2)
      CALL MPEQ (MPM2, UU)
      ! CMP<
      ! CMP>                 VV = V*(1+C4/TEMP)
      CALL MPDIV (C4, TEMP, MPM1)
      MPD1 = 1
      CALL MPDMC (MPD1, 0, MPM3)
      CALL MPADD (MPM3, MPM1, MPM2)
      CALL MPMUL (V, MPM2, MPM1)
      CALL MPEQ (MPM1, VV)
      ! CMP<
      RETURN
      ...
      ! CMP>     UU = ZERO
15    CONTINUE
      CALL MPDMC (ZERO, 0, UU)
      ! CMP<
      ! CMP>                 VV = ZERO
      CALL MPDMC (ZERO, 0, VV)
      ! CMP<
      RETURN
   END  SUBROUTINE MP_ZRPQLG
   ! c-----------------------------------------------------------------------
   ! New MP version ZRPQLH

   SUBROUTINE MP_ZRPQLH (NN,U,V,P,Q,RA,RB)
      INTEGER    NN
      REAL U(21),V(21),P(21,NN), Q(21,NN),RA(21),RB(21)
      INTEGER       I
      ...
      DO 5 I=3,NN
         ! CMP>                    C = P(I)-U*RA-V*RB
         CALL MPMUL (U, RA, MPM1)
         CALL MPSUB (P(1,I), MPM1, MPM2)
         CALL MPMUL (V, RB, MPM1)
         CALL MPSUB (MPM2, MPM1, MPM3)
         CALL MPEQ (MPM3, C)
         ! CMP<
...
5     CONTINUE
      RETURN
   END SUBROUTINE MP_ZRPQLH

! new MP version    ZRPQLI

   SUBROUTINE MP_ZRPQLI (RA,B1,C,SR,SI,RLR,RLI)
      REAL*8 ZERO, ONE, TWO
      ! CMP<
      REAL B1(21), RLR(21), RLI(21), P(21), QP(21), RK(21), QK(21)
      REAL SVK(21), SR(21), SI(21), U(21), V(21), RA(21), RB(21), C(21)
      REAL D(21), A1(21), A2(21), A3(21), A6(21), A7(21), E(21),  F(21)
      REAL G(21), H(21), SZR(21), SZI(21), RLZR(21), RLZI(21), ETA(21)
      REAL ARE(21), RMRE(21), MPM1(21), MPM2(21), MPM3(21)
      INTEGER MPI1
      LOGICAL MPL1
      ! C
      ! CMP<
      ...
      ! c                                    ZEROS C/A
      ! c                                  FIRST EXECUTABLE STATEMENT
      ! CMP>                 IF (RA.NE.ZERO) GO TO 10
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (RA, MPM1, MPI1)
      MPL1 = MPI1 .NE. 0
      IF (MPL1) THEN
         GO TO 10
      ENDIF
      ! CMP<
      ! CMP>                 SR = ZERO
      CALL MPDMC (ZERO, 0, SR)
      ! CMP<
      ! CMP>                 IF (B1.NE.ZERO) SR = -C/B1
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (B1, MPM1, MPI1)
      MPL1 = MPI1 .NE. 0
      IF (MPL1) THEN
         CALL MPDIV (C, B1, MPM1)
         CALL MPEQ (MPM1, MPM2)
         MPM2(1) = - MPM2(1)
         CALL MPEQ (MPM2, SR)
      ENDIF
      ! CMP<
      ! CMP>                 RLR = ZERO
      CALL MPDMC (ZERO, 0, RLR)
      ! CMP<
      ! CMP>     SI = ZERO
5     CONTINUE
      CALL MPDMC (ZERO, 0, SI)
      ! CMP<
      ! CMP>                 RLI = ZERO
      CALL MPDMC (ZERO, 0, RLI)
      ! CMP<
      RETURN
      ! CMP>     IF (C.NE.ZERO) GO TO 15
10    CONTINUE
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (C, MPM1, MPI1)
      MPL1 = MPI1 .NE. 0
      IF (MPL1) THEN
         GO TO 15
      ENDIF
      ...
      ! CMP>     RB = B1/TWO
15    CONTINUE
      CALL MPDIVD (B1, TWO, 0, MPM1)
      CALL MPEQ (MPM1, RB)
      ! CMP<
      ! CMP>                 IF (ABS(RB).LT.ABS(C)) GO TO 20
      CALL MPEQ (RB, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPEQ (C, MPM2)
      MPM2(1) = ABS (MPM2(1))
      ...
      ENDIF
      ! CMP<
      ! CMP>                 E = ONE-(RA/RB)*(C/RB)
      CALL MPDIV (RA, RB, MPM1)
      CALL MPDIV (C, RB, MPM2)
      CALL MPMUL (MPM1, MPM2, MPM3)
      CALL MPDMC (ONE, 0, MPM2)
      CALL MPSUB (MPM2, MPM3, MPM1)
      CALL MPEQ (MPM1, E)
      ! CMP<
      ! CMP>                 D = SQRT(ABS(E))*ABS(RB)
      CALL MPEQ (E, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPSQRT (MPM1, MPM2)
      CALL MPEQ (RB, MPM1)
...
20    CONTINUE
      ...
      CALL MPEQ (C, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPDIV (RB, MPM1, MPM2)
      CALL MPMUL (RB, MPM2, MPM1)
      CALL MPSUB (MPM1, E, MPM2)
      CALL MPEQ (MPM2, E)
      ! CMP<
      ! CMP>                 D = SQRT(ABS(E))*SQRT(ABS(C))
      CALL MPEQ (E, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPSQRT (MPM1, MPM2)
      CALL MPEQ (C, MPM1)
      MPM1(1) = ABS (MPM1(1))
      CALL MPSQRT (MPM1, MPM3)
      CALL MPMUL (MPM2, MPM3, MPM1)
      CALL MPEQ (MPM1, D)
      ! CMP<
      ! CMP>     IF (E.LT.ZERO) GO TO 30
25    CONTINUE
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (E, MPM1, MPI1)
      MPL1 = MPI1 .LT. 0
      IF (MPL1) THEN
         GO TO 30
      ENDIF
      ! CMP<
      ! c                                 REAL ZEROS
      ! CMP>                 IF (RB.GE.ZERO) D = -D
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (RB, MPM1, MPI1)
      MPL1 = MPI1 .GE. 0
      IF (MPL1) THEN
         CALL MPEQ (D, MPM1)
         MPM1(1) = - MPM1(1)
      ...
      CALL MPEQ (RB, MPM1)
      MPM1(1) = - MPM1(1)
      CALL MPADD (MPM1, D, MPM2)
      CALL MPDIV (MPM2, RA, MPM1)
      CALL MPEQ (MPM1, RLR)
      ! CMP<
      ! CMP>                 SR = ZERO
      CALL MPDMC (ZERO, 0, SR)
      ! CMP<
      ! CMP>                 IF (RLR.NE.ZERO) SR = (C/RLR)/RA
      CALL MPDMC (ZERO, 0, MPM1)
      CALL MPCPR (RLR, MPM1, MPI1)
      MPL1 = MPI1 .NE. 0
      IF (MPL1) THEN
         CALL MPDIV (C, RLR, MPM1)
         CALL MPDIV (MPM1, RA, MPM2)
         CALL MPEQ (MPM2, SR)
      ENDIF
      ! CMP<
      GO TO 5
