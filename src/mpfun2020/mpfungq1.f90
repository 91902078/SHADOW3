...
!  mp_gamma, mp_gammainc, mp_hypot, mp_init, mp_initvr, mp_initvz, &
  mp_gamma, mp_hypot, mp_init, mp_initvr, mp_initvz, &
...
!  and the value is set to the "NAN" of MPFR.
  ...
  subroutine mp_initvr (ra, nbt)
    ...
    ra%mpr(1) = nbt
#if defined(_MSC_VER) 
!skip sign, due to mpfr.dll structure member not align in 8 bytes
    ra%mpr(2) = mpnan4       !correct
    ra%mpr(3) = loc (ra%mpr(3)) + 8
#else
    ra%mpr(2) = 1          
    ra%mpr(3) = mpnan
...
#if defined(_MSC_VER) 
    za%mpc(2) = mpnan4
    za%mpc(3) = loc (za%mpc(3)) + 8
    za%mpc(l1+2) = mpnan4
    za%mpc(l1+3) = loc (za%mpc(l1+3)) + 8
#else
    za%mpc(2) = 1   !sign
    za%mpc(3) = mpnan
...
#if defined(_MSC_VER) 
    if (.not. btest(rb%mpr(1),33)) then ! +:bit 32=1, bit 33-63=0  -:32-63=1
#else
    if (rb%mpr(2) == 1) then
#endif    
...
!   in the system parameters at the start of module MPFUNA.
  ...
  character(1) chr1(ln+1)
  character(32) cform1, cform2
  type (mp_real) a

  call mp_fixlocr (a)
  call mp_eform (a, ln+1, nd, chr1)
  ...
  if (ln <= mpoutln) then
    write (iu, fmt = cform1) (chr1(i), i = 1, ln+1)
  elseif (mod (ln+1, mpoutln) == 0) then
    ln1 = mpoutln * ((ln+1) / mpoutln) - mpoutln
    write (iu, fmt = cform2) (chr1(i), i = 1, ln1)
    write (iu, fmt = cform1) (chr1(i), i = ln1 + 1, ln+1)
  else
    ln1 = mpoutln * ((ln+1) / mpoutln)
    write (iu, fmt = cform2) (chr1(i), i = 1, ln1)
    write (iu, fmt = cform1) (chr1(i), i = ln1 + 1, ln+1)
  endif
